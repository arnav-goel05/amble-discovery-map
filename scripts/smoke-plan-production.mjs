...
const store = fs.mkdtempSync(
  path.join(os.tmpdir(), "whats-here-production-smoke-"),
);
...
function start() {
  child = spawn(
    process.execPath,
    ["scripts/serve-app.cjs", "--host", "127.0.0.1", "--port", String(port)],
...
try {
  ...
  assert.equal(
    (await (await fetch(`${origin}/api/plans/${plan.id}`)).json()).title,
  ...
  const compressedTileset = await fetch(backgroundTilesetUrl, {
  ...
  const index = fs
    ...
    .find((file) => /^(?:index|main)\..+\.js$/.test(file));
  assert.ok(index, "a hashed public application script must exist");
  const range = await fetch(`${origin}/assets/${index}`, {
  ...
  const buildingTileName = fs
    .readdirSync(tileDirectory)
    .find((file) => file.endsWith(".b3dm"));
  assert.ok(
    buildingTileName,
    "a production building tile must exist for cache verification",
  );
  ...
  assert.equal(
    buildingTile.headers.get("cache-control"),

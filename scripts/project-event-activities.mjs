...
const requested =
  option("--run") ??
  (existsSync(pointer) ? readJson(pointer).snapshotId : null);
if (!requested) throw new Error("Pass --run <run-id-or-directory>.");
const direct = resolve(requested);
const runDir = existsSync(direct)
  ? direct
  : join(root, "outputs/event-pipeline", requested);
const eventsPath = join(runDir, "normalized/events.json");
if (!existsSync(eventsPath))
  throw new Error(`Normalized events not found: ${eventsPath}`);

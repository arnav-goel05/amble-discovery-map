...
async function expectR2(url, { json = false } = {}) {
  ...
  if (source !== "r2") throw new Error(`Expected R2 delivery for ${url}; received ${source || "no source header"}`);
...
async function verifyContent(url, depth = 0) {
  ...
  if (!url.pathname.endsWith(".json")) {
    await expectR2(url);
    const rangeResponse = await fetch(url, { headers: { range: "bytes=0-99" } });
    if (rangeResponse.status !== 206 || rangeResponse.headers.get("x-amble-tile-source") !== "r2") {
      throw new Error(`Expected an R2 byte-range response for ${url}; received ${rangeResponse.status}`);
    }
    return url;
  }
...
console.log(JSON.stringify({ complete: true, origin: origin.href, results }, null, 2));

...
function publicMetadata(snapshot) {
  const sourceHealth = Object.fromEntries(
    Object.entries(snapshot.sourceHealth || {}).map(([id, health]) => [
      id,
      {
        status: [
          "success",
...
function approvedSnapshotApiPlugin({
  ...
  const middleware = async (request, response, next) => {
    ...
    try {
      ...
      if (url.pathname === "/api/snapshot") {
        const envelope = successEnvelope(publicMetadata(active), {
        ...
        return request.method === "HEAD"
          ? (sendJson(response, 200, envelope), undefined)
          : sendJson(response, 200, envelope, { cacheControl: "no-cache" });
      }
      ...
      const file = snapshots.resolveActiveSnapshotAsset({
        root,
        snapshotId,
        reference,
      });
      if (reference === active.tilesetRef) {
        const tileset = publicTileset(
      ...
      return sendPublicError(response, error);
    }
  };
  return {

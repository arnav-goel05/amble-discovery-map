...
} from "./event-source-collector.mjs";
import {
...
} from "./lib/location-evidence.mjs";
...
import {
  progressResponse as stateProgressResponse,
  renderStatus as renderPipelineStatus,
  statusSummary,
...
import {
  createTraceWriter,
  redactTraceValue,
} from "./lib/event-sources/trace.mjs";
import {
  parseArgs,
  parseManifest,
...
const resolutionCachePath = () =>
  process.env.EVENT_PIPELINE_RESOLUTION_CACHE
    ? resolve(process.env.EVENT_PIPELINE_RESOLUTION_CACHE)
    : join(OUTPUT_ROOT, "venue-resolution-cache.json");
const aliasRegistryPath = () =>
  process.env.EVENT_PIPELINE_ALIAS_REGISTRY
    ? resolve(process.env.EVENT_PIPELINE_ALIAS_REGISTRY)
    : join(ROOT, "data/venue-alias-registry.json");
const venueRecoveryPath = (runDir) =>
...
const adminDatabasePath = () =>
  process.env.ADMIN_DATABASE_PATH
    ? resolve(process.env.ADMIN_DATABASE_PATH)
    : process.env.EVENT_PIPELINE_OUTPUT_ROOT
      ? join(OUTPUT_ROOT, "admin.sqlite")
      : join(ROOT, "outputs/admin/admin.sqlite");
...
function readResolutionCache() {
  ...
  if (cache.schemaVersion !== "1.0" || !Array.isArray(cache.entries))
    fail("Invalid runtime venue resolution cache");
  return cache;
}
...
function recoveryEvidenceRef(runDir, venueId) {
  const index = recoveryRecords(runDir).findIndex(
    (record) => record.venueId === venueId,
  );
  return index < 0
    ? null
    : `normalized/venue-recovery-evidence.json#/records/${index}`;
}
...
function selectDeterministicOneMapAddress(address, response) {
  const exact = (response?.results ?? []).filter(
    (row) =>
      normalizeText(row.address) === normalizeText(address) ||
  ...
  const namedMatches = (response?.results ?? []).filter((row) => {
    ...
    return (
      name.length >= 6 &&
      (normalizedAddress.includes(name) || name.includes(normalizedAddress))
    );
  });
  ...
  const postalMatches = postalCode
    ? (response?.results ?? []).filter((row) => row.postalCode === postalCode)
    : [];
  const equivalentPins = consolidateCoordinateCandidates(
  ...
  if (postalMatches.length > 1 && equivalentPins.length === 1)
    return equivalentPins[0].row;
...
async function enrichRecoveryCoordinates(normalized, geocode = queryOneMap) {
  if (normalized.notMappableEvidence || !normalized.addressCandidates.length)
    return normalized;
      ...
      coordinateCandidates: [
        {
          ...
          recordRef: response.requestUrl ?? null,
          evidenceField: response.selectedQuery ?? address,
        },
      ],
    };
  }
...
function classifyNonBuildingRecovery(normalized) {
  ...
  const isNonBuilding =
    /\bmrt(?: station)? exit\s*[a-z0-9-]*\b|\b(?:mrt )?(?:platform|gantry|passage)\b/i.test(
      location,
    );
  ...
  const sourceUrls = [
    ...new Set(
      [
        ...(normalized.evidenceInspected ?? []),
        ...(normalized.supplementalEvidence ?? []),
      ]
        .map((item) => item.url)
  ...
  return {
    ...normalized,
    coordinateCandidates: [],
    notMappableEvidence: { reasonCode: "no_target_building", sourceUrls },
  };
}
  ...
  return [
    ...new Set(
      events.flatMap((event) => [
        event.eventUrl,
        ...(event.sources ?? []).map((source) => source.sourceUrl),
      ]),
    ),
  ].filter((url) => typeof url === "string" && /^https?:\/\//.test(url));
}

function applyDeterministicEventClassification(
  runId,
  venueId,
  venue,
  localRow,
  ...
  const record = {
    ...
    postalCodes: [],
    coordinateCandidates: [],
    evidenceInspected: sourceUrls.map((url) => ({
      sourceType: "host_or_authority",
      label: "Normalized source event with explicit Locations list",
      url,
    })),
    notMappableEvidence: { reasonCode: "multi_venue", sourceUrls },
    recordedAt: new Date().toISOString(),
  };
  const previousIndex = (envelope.records ?? []).findIndex(
...
function validateVenueRecoveryEvidence(value, expectedVenue = null) {
  if (!value || typeof value !== "object")
    fail("Venue recovery evidence must be a JSON object");
  ...
  if (unknownFields.length)
    fail(
      `Venue recovery evidence contains unsupported fields: ${unknownFields.join(", ")}; use the supplied template fields exactly`,
    );
  if (value.schemaVersion !== "1.0")
    fail("Venue recovery evidence requires schemaVersion 1.0");
  if (
    expectedVenue &&
    value.venue &&
    normalizeText(value.venue) !== normalizeText(expectedVenue)
  )
    fail("Venue recovery evidence does not match the requested venue");
  ...
  const notMappableEvidence = submittedNotMappableEvidence
    ? {
        ...
        sourceUrls: [
          ...new Set(
            (submittedNotMappableEvidence.sourceUrls ?? [])
              .map((item) =>
                typeof item === "string"
                  ? item.trim()
  ...
  if (notMappableEvidence) {
    if (
      ![
        ...
        "multi_venue",
        "no_target_building",
      ].includes(notMappableEvidence.reasonCode)
    )
      fail("Invalid venue recovery notMappableEvidence reasonCode");
    if (
      !Array.isArray(submittedNotMappableEvidence.sourceUrls) ||
      notMappableEvidence.sourceUrls.length === 0
    )
      fail(
        "Venue recovery notMappableEvidence requires authoritative sourceUrls",
      );
  }
  ...
  if (
    postalCodes.some((item) =>
      outsideSingapore
        ? !/^[a-z0-9][a-z0-9 -]{1,11}$/i.test(item)
        : !/^\d{6}$/.test(item),
    )
  ) {
    fail(
      outsideSingapore
        ? "Outside-Singapore recovery postalCodes must contain valid postal text"
        : "Venue recovery postalCodes must contain six-digit Singapore postal codes",
    );
  }
  const coordinateCandidates = (value.coordinateCandidates ?? []).map(
    (item) => {
      const urlCoordinate = item?.url ? extractCoordinates(item.url)[0] : null;
      return {
        lat: Number(item?.lat ?? item?.latitude ?? urlCoordinate?.lat),
        lng: Number(item?.lng ?? item?.longitude ?? urlCoordinate?.lng),
        source:
          item?.source ?? item?.sourceType ?? "authoritative_web_recovery",
        recordRef: item?.recordRef ?? item?.url ?? null,
        evidenceField:
          item?.evidenceField ??
          item?.label ??
          "authoritative venue recovery evidence",
      };
    },
  );
  const isRealWorldCoordinate = (item) =>
    Number.isFinite(item.lat) &&
    Number.isFinite(item.lng) &&
    Math.abs(item.lat) <= 90 &&
    Math.abs(item.lng) <= 180 &&
    !(item.lat === 0 && item.lng === 0);
  const isSingaporeCoordinate = (item) =>
    item.lat >= 1.13 &&
    item.lat <= 1.49 &&
    item.lng >= 103.55 &&
    item.lng <= 104.15;
  if (
    coordinateCandidates.some(
      (item) =>
        !isRealWorldCoordinate(item) ||
        (!outsideSingapore && !isSingaporeCoordinate(item)),
    )
  ) {
    fail(
      outsideSingapore
        ? "Outside-Singapore recovery coordinates must be valid non-sentinel world coordinates"
        : "Venue recovery coordinates must be finite and within Singapore",
    );
  }
  const submittedEvidence = value.evidenceInspected ?? [];
  if (!Array.isArray(submittedEvidence) || submittedEvidence.length < 2)
    fail(
      "Venue recovery requires both venue-official and host/authority pages",
    );
  const isSearchHost = (hostname) =>
    /(?:^|\.)(?:google\.[a-z.]+|bing\.com|duckduckgo\.com|search\.yahoo\.com)$/i.test(
      hostname,
    );
  const sourceTypes = new Set(),
    evidenceInspected = [],
    supplementalEvidence = [];
  const coordinateRefs = new Set(
    coordinateCandidates.map((item) => item.recordRef).filter(Boolean),
  );
  for (const evidence of submittedEvidence) {
    if (!evidence?.label || !evidence?.url)
      fail(
        "Every venue recovery evidence item requires a label and inspected URL",
      );
    if (
      !evidence?.query ||
      !evidence?.outcome ||
      ...
      fail(
        "Every venue recovery evidence item requires its actual query, outcome, and checkedAt timestamp",
      );
    let parsed;
    try {
      parsed = new URL(evidence.url);
    } catch {
      fail(`Invalid venue recovery evidence URL: ${evidence.url}`);
    }
    if (!["http:", "https:"].includes(parsed.protocol))
      fail("Venue recovery evidence URL must use HTTP or HTTPS");
    const listingProvider =
      /(?:^|\.)(?:catch\.sg|sistic\.com\.sg|sgculturepass\.gov\.sg)$/i.test(
        parsed.hostname,
      );
    if (evidence.sourceType === "venue_official" && listingProvider) {
      fail(
        "Catch.sg, SISTIC, and SG Culture Pass listing pages must use sourceType host_or_authority; open the actual venue or operator website for venue_official evidence",
      );
    }
    const isOneMap = parsed.hostname === "www.onemap.gov.sg";
    const coreType =
      ["venue_official", "host_or_authority"].includes(evidence.sourceType) &&
      !isSearchHost(parsed.hostname) &&
      !isOneMap;
    if (coreType) {
      sourceTypes.add(evidence.sourceType);
      evidenceInspected.push(evidence);
    } else if (
      coordinateRefs.has(evidence.url) ||
      isOneMap ||
      (["address_authority", "coordinate_authority"].includes(
        evidence.sourceType,
      ) &&
        !isSearchHost(parsed.hostname))
    ) {
      ...
      fail(
        "Venue recovery evidence must be an inspected authoritative page, not a search-result URL",
      );
    }
  }
  if (
    !sourceTypes.has("venue_official") ||
    !sourceTypes.has("host_or_authority")
  )
    fail(
      "Venue recovery must cover venue_official and host_or_authority paths",
    );
  const officialPostalCodes = [
    ...new Set(
      evidenceInspected
        .filter((evidence) => evidence.sourceType === "venue_official")
        .flatMap(
          (evidence) =>
            String(evidence.outcome ?? "").match(/\b\d{6}\b/g) ?? [],
        ),
    ),
  ];
  if (
    officialPostalCodes.length === 1 &&
    postalCodes.length &&
    !postalCodes.includes(officialPostalCodes[0])
  ) {
    fail(
      `Venue recovery postalCodes conflict with the venue-official evidence (${officialPostalCodes[0]}); remove contaminated source-page location clues`,
    );
  }
  if (
    officialPostalCodes.length === 1 &&
    addressCandidates.some((address) => {
      const submittedPostal = String(address).match(/\b\d{6}\b/)?.[0];
      return submittedPostal && submittedPostal !== officialPostalCodes[0];
    })
  ) {
    fail(
      `Venue recovery addressCandidates conflict with the venue-official evidence (${officialPostalCodes[0]}); replace contaminated source-page location clues`,
    );
  }
  for (const candidate of coordinateCandidates) {
    const matchedEvidence = submittedEvidence.find(
      (evidence) => evidence.url === candidate.recordRef,
    );
    if (
      matchedEvidence &&
      /\b(?:no|without)\s+(?:visible\s+|usable\s+|published\s+)?(?:map\s+)?(?:pin|coordinates?|map location)\b|\b(?:pin|coordinates?|map location)\s+(?:is\s+|are\s+)?(?:not\s+)?(?:shown|available|provided|present|exposed|found)\b/i.test(
        matchedEvidence.outcome,
      )
    ) {
      fail(
        "A page recorded as exposing no map pin or coordinate cannot be used as the coordinate recordRef",
      );
    }
    if (/onemap/i.test(candidate.evidenceField) && candidate.recordRef) {
      let hostname = "";
      try {
        hostname = new URL(candidate.recordRef).hostname;
      } catch {}
      if (hostname !== "www.onemap.gov.sg")
        fail(
          "OneMap-derived coordinates must use the onemap-geocode requestUrl as recordRef",
        );
    }
  }
  return {
    ...
    coordinateCandidates,
    evidenceInspected,
    ...(supplementalEvidence.length ? { supplementalEvidence } : {}),
    notMappableEvidence,
  };
}

function validateNotMappableAgainstLocalCandidates(
  recovery,
  venueName,
  localRow,
) {
  ...
  const stableVenueName = normalizeText(venueName)
    .replace(
      /\b(?:meeting|meet up|pickup|pick up|starting|start) point\b/g,
      " ",
    )
    .replace(/\s+/g, " ")
    .trim();
  ...
  const exactCandidate = (localRow?.alternatives ?? []).find((candidate) => {
    ...
    return (
      ...
      (candidateName === stableVenueName ||
        stableVenueName.includes(candidateName)) &&
      Array.isArray(candidate?.gmlIds) &&
      candidate.gmlIds.length === 1
    );
  });
  if (exactCandidate) {
    fail(
      `Venue recovery cannot classify ${venueName} as ${reasonCode}: local OneMap evidence contains exact building candidate ${exactCandidate.name} (${exactCandidate.gmlIds[0]})`,
    );
  }
}
...
function readPipelineConfig() {
  ...
  if (
    !["1.0", "2.0"].includes(stored.schemaVersion) ||
    stored.timezone !== "Asia/Singapore"
  )
    fail("Invalid event pipeline configuration");
  const config = {
    ...stored,
    sources: (stored.sources ?? []).map((source) => {
      ...
      return {
        ...
        sourceRole:
          evidenceRole === "editorial" ? "discovery" : "authoritative",
  ...
  if (
    !Number.isInteger(config.windowDaysAfterStart) ||
    config.windowDaysAfterStart < 0
  )
    fail("Invalid windowDaysAfterStart");
  const requestPolicy = config.requestPolicy ?? {};
  if (
    !Number.isInteger(requestPolicy.timeoutMs) ||
    requestPolicy.timeoutMs < 100 ||
    requestPolicy.timeoutMs > 60_000 ||
    !Number.isInteger(requestPolicy.maxAttempts) ||
    requestPolicy.maxAttempts < 1 ||
    requestPolicy.maxAttempts > 5 ||
    !Number.isInteger(requestPolicy.initialBackoffMs) ||
    requestPolicy.initialBackoffMs < 0 ||
    !Number.isInteger(requestPolicy.maximumBackoffMs) ||
    requestPolicy.maximumBackoffMs < requestPolicy.initialBackoffMs
  ) {
  ...
  for (const source of config.sources ?? []) {
    if (
      ...
      fail("Each source requires a unique name, adapterId, and version");
    ids.add(source.adapterId);
    if (
      !["direct", "editorial", "unavailable"].includes(source.evidenceRole) ||
      !["enabled", "disabled"].includes(source.operatingState) ||
      !["authoritative", "discovery"].includes(source.sourceRole) ||
      !["required", "disabled"].includes(source.operatingMode) ||
      source.enabled !== (source.operatingMode !== "disabled") ||
      !Number.isInteger(source.collectionOrder) ||
      collectionOrders.has(source.collectionOrder)
    ) {
      fail(
        `Invalid role, mode, enabled state, or collection order for ${source.name}`,
      );
    }
    ...
    if (source.evidenceRole !== "editorial") {
      if (
        ...
        fail(`Invalid authoritative precedence for ${source.name}`);
      precedences.add(source.precedence);
    } else if (
      source.precedence !== null ||
      source.editorialPolicy?.version !== "2.0"
    )
      fail(`Invalid editorial policy/precedence for ${source.name}`);
    for (const endpoint of [source.listing, source.detail]) {
      if (!endpoint?.url || !["GET", "POST"].includes(endpoint.method))
        fail(`Invalid endpoint configuration for ${source.name}`);
      try {
        if (new URL(endpoint.url).protocol !== "https:") throw new Error();
      } catch {
        fail(`Source endpoints must use HTTPS: ${source.name}`);
      }
    }
    try {
      validateSourcePolicy(source);
    } catch (error) {
      fail(`Invalid provider policy for ${source.name}: ${error.message}`);
    }
  }
  return config;
}

function singaporeWindow(
  dateText,
  daysAfterStart = readPipelineConfig().windowDaysAfterStart,
) {
...
function acquireLock(runId) {
  ...
  try {
    ...
    if (error.code === "EEXIST") {
      ...
      try {
        ...
        fail(`Unreadable pipeline lock requires manual review: ${LOCK_PATH}`);
      }
      const age = Date.now() - new Date(lock.startedAt).valueOf();
      let alive = true;
      if (
        lock.owner?.host === hostname() &&
      ...
      fail(
        `Pipeline lock already exists for run ${lock.runId ?? "unknown"} at ${LOCK_PATH}`,
      );
    }
    throw error;
  }
  writeFileSync(
    fd,
    `${JSON.stringify({ runId, startedAt: new Date().toISOString(), owner: { host: hostname(), pid: process.pid } }, null, 2)}\n`,
  );
  closeSync(fd);
}
...
function invalidateResumeArtifacts(runDir, state, run) {
  ...
  while (changed) {
    ...
    for (const [reference, metadata] of Object.entries(run.artifacts)) {
      if (
        metadata.status === "invalidated" ||
        !(metadata.inputSha256 ?? []).some((hash) => oldHashes.has(hash))
      )
        continue;
      metadata.status = "invalidated";
      invalidated.add(reference);
  ...
  for (const source of Object.values(state.sources)) {
    if (
      (source.artifactRefs ?? []).some((reference) =>
        invalidated.has(reference),
      )
    )
  ...
  if (
    (state.normalization.artifactRefs ?? []).some((reference) =>
      invalidated.has(reference),
    ) ||
    Object.values(state.sources).some((source) => source.status === "pending")
  ) {
    state.normalization = {
      status: "pending",
      counts: null,
    ...
    for (const venue of Object.values(state.venues))
      for (const stage of STAGES) {
        if (
          venue.stages[stage].outputRef &&
          invalidated.has(venue.stages[stage].outputRef)
        ) {
          const index = STAGES.indexOf(stage);
          for (const downstream of STAGES.slice(index))
            venue.stages[downstream] = {
              status: "pending",
              outputRef: null,
              error: null,
            };
...
function resume(options) {
  ...
  const manifestSnapshot = readFileSync(
    join(runDir, run.manifestSnapshot.path),
  );
  const configSnapshot = readFileSync(
    join(runDir, run.adapterDefinitionsSnapshot.path),
  );
  if (sha(manifestSnapshot) !== run.manifestSnapshot.sha256)
    fail("Resume rejected: manifest snapshot hash mismatch");
  if (sha(configSnapshot) !== run.adapterDefinitionsSnapshot.sha256)
    fail("Resume rejected: pipeline configuration snapshot hash mismatch");
  if (sha(readFileSync(CONFIG_PATH)) !== run.adapterDefinitionsSnapshot.sha256)
    fail(
  ...
  process.stdout.write(
    `${JSON.stringify({ ...progressResponse(state), resumed: true, invalidatedArtifacts }, null, 2)}\n`,
  );
...
function artifactPath(runDir, reference) {
  if (
    ...
    fail(`Invalid artifact reference: ${reference}`);
...
function start(options) {
  ...
  if (!manifest.sources.some((source) => source.enabled))
    fail("Pipeline configuration contains no enabled sources");
  ...
  try {
    ...
    const adapters = manifest.sources
      .filter((source) => source.enabled)
      .map(({ adapterId, version }) => ({
        id: adapterId,
        version,
        definitionSha256,
      }))
      ...
      sources: Object.fromEntries(
        manifest.sources.map((source) => [
          source.name,
          {
            ...
            status:
              source.operatingMode === "disabled" ? "disabled" : "pending",
            counts: null,
            artifactRefs: [],
            error:
              source.operatingMode === "disabled"
                ? `Source unavailable: ${source.unavailableReason ?? "operator_disabled"}`
                : null,
            blockerReasonCode:
              source.operatingMode === "disabled"
                ? (source.unavailableReason ?? "operator_disabled")
                : null,
          },
        ]),
      ),
    ...
    for (const source of manifest.sources)
      pipelineTrace(runDir, {
        stage: "configuration",
        action: "source_configured",
        outcome: source.operatingMode === "disabled" ? "disabled" : "success",
    ...
    if (!options.quiet) {
      process.stdout.write(
        `${JSON.stringify({ runDir, ...progressResponse(loadState(runId)) }, null, 2)}\n`,
      );
  ...
  if (result.status === "success") {
    ...
    for (const key of required)
      if (!Number.isInteger(result.counts?.[key]) || result.counts[key] < 0)
        fail(`Invalid source count: ${key}`);
    const c = result.counts;
    if (
      c.sourceRecordsReceived !==
      c.invalidSourceRecords + c.processedSourceRecords
    )
      fail("Source record accounting does not reconcile");
    ...
    if (
      ...
      fail("completion.pagesVisited must account for every page");
    if (result.completion.sourceRecordsDiscovered !== c.sourceRecordsReceived)
      fail(
        "completion.sourceRecordsDiscovered must match sourceRecordsReceived",
      );
    const rendered = result.completion.providerReportedTotal === null;
    if (rendered && result.completion.derivedTotal !== c.sourceRecordsReceived)
      fail("completion.derivedTotal must match sourceRecordsReceived");
    if (
      !rendered &&
      (!Number.isInteger(result.completion.providerReportedTotal) ||
        result.completion.providerReportedTotal !== c.sourceRecordsReceived)
    )
      fail("completion.providerReportedTotal must match sourceRecordsReceived");
    if (
      !Array.isArray(result.completion.pageRecordCounts) ||
      result.completion.pageRecordCounts.length !== c.pages ||
      result.completion.pageRecordCounts.some(
        (count) => !Number.isInteger(count) || count < 0,
      )
    )
      fail(
        "completion.pageRecordCounts must provide a non-negative count for every page",
      );
    if (
      !rendered &&
      result.completion.pageRecordCounts.reduce(
        (sum, count) => sum + count,
        0,
      ) !== c.sourceRecordsReceived
    )
    ...
    if (
      ...
      fail("A zero-record success requires completion.zeroResultConfirmed");
    if (
      !Array.isArray(result.sourceRecordRefs) ||
      result.sourceRecordRefs.length !== c.sourceRecordsReceived
    )
      fail("sourceRecordRefs must account for every received source record");
    if (
      !Array.isArray(result.invalidSourceRecordRefs) ||
    ...
    const partition = [
      ...result.invalidSourceRecordRefs,
      ...result.processedSourceRecordRefs,
    ];
    if (
      received.size !== result.sourceRecordRefs.length ||
      new Set(partition).size !== partition.length ||
      partition.length !== received.size ||
      partition.some((ref) => !received.has(ref))
    ) {
      fail(
    ...
    if (!EXTERNAL_BLOCKER_CODES.has(result.blockerReasonCode)) {
      fail(
        `Blocked source requires a genuine external blockerReasonCode: ${[...EXTERNAL_BLOCKER_CODES].join(", ")}`,
      );
    }
...
function baseArtifactRef(recordRef) {
  if (typeof recordRef !== "string" || !recordRef.includes("#/"))
    fail(`Invalid source record pointer: ${recordRef}`);
...
function jsonPointer(value, pointer) {
  ...
  if (typeof pointer !== "string" || !pointer.startsWith("/"))
    fail(`Invalid JSON pointer: ${pointer}`);
  return pointer
    .slice(1)
    .split("/")
...
function parseEventBoundary(value, endOfDay = false) {
  ...
  const timestamp = dayMonthYear
    ? Date.parse(
        `${dayMonthYear[1]} ${dayMonthYear[2]} ${dayMonthYear[3]} ${endOfDay ? "23:59:59" : "00:00:00"} +0800`,
      )
    : Date.parse(trimmed);
...
function eventFinalBoundary(event) {
  ...
  if (
    ["selectable", "recurring", "anytime", "unverified"].includes(
      event.schedule?.kind,
    )
  )
    return null;
...
function validateNormalizedSemantics(
  runDir,
  normalizedEvents,
  result,
  runWindow,
  ...
  if (result.counts.acceptedPrimary !== events.length)
    fail("acceptedPrimary must equal normalized event records");
  const ids = new Set();
  const byId = new Map();
  for (const event of events) {
    if (typeof event.id !== "string" || !event.id || ids.has(event.id))
      fail("Normalized events require unique non-empty ids");
    ids.add(event.id);
    ...
    if (
      finalBoundary !== null &&
      finalBoundary < Date.parse(runWindow.start) &&
      event.lifecycleState !== "archived"
    )
      fail(
        `Normalized event ${event.id} ended before the run and was not archived`,
      );
  }
  ...
  for (const branch of result.venueBranches) {
    if (
      !branch.id ||
      typeof branch.venue !== "string" ||
      !branch.venue.trim() ||
      !Array.isArray(branch.eventIds) ||
      branch.eventIds.length === 0
    )
      fail("Each venue branch needs id, venue, and eventIds");
    for (const eventId of branch.eventIds) {
      const event = byId.get(eventId);
      if (!event) fail(`Venue branch references unknown event: ${eventId}`);
      if (branched.has(eventId))
        fail(`Event appears in multiple venue branches: ${eventId}`);
      if (event.isOnline === true)
        fail(`Online event must not enter a venue branch: ${eventId}`);
      if (normalizeText(event.venue) !== normalizeText(branch.venue))
        fail(`Venue branch ${branch.id} mixes events from different venues`);
      branched.add(eventId);
    }
  }
  const expectedBranched = events
    .filter(
      (event) =>
        event.isOnline !== true &&
        (event.lifecycleState ?? "active") === "active" &&
        (event.publicPlacement !== "off_map" ||
          event.mappingStatus === "pending_review"),
    )
    .map((event) => event.id);
  if (
    branched.size !== expectedBranched.length ||
    expectedBranched.some((id) => !branched.has(id))
  )
    fail("Venue branches must exactly partition physical normalized events");
}
...
function reconcileNormalizedVenueBranches(previousVenues, branches) {
  ...
  const venues = Object.fromEntries(
    branches.map((branch) => {
      ...
      const unchanged =
        previous &&
        normalizeText(previous.venue) === normalizeText(branch.venue) &&
...
function validateStageEventIds(stage, expectedEventIds, result) {
  ...
  if (
    !Array.isArray(inputEventIds) ||
    inputEventIds.length !== expectedEventIds.length ||
    new Set(inputEventIds).size !== inputEventIds.length ||
    expectedEventIds.some((id) => !inputEventIds.includes(id))
  ) {
...
function validateHighlightArtifacts(result, root = ROOT) {
  ...
  if (typeof tilesetRef !== "string" || !tilesetRef.trim())
    fail("Successful highlight requires poiTilesetUrl");
  const tilesetPath = resolve(root, tilesetRef.replace(/^\//, ""));
  if (
    !tilesetPath.startsWith(
      `${resolve(root)}${process.platform === "win32" ? "\\" : "/"}`,
    ) ||
    !existsSync(tilesetPath)
  )
    fail(`Successful highlight tileset does not exist: ${tilesetRef}`);
  ...
  if (
    typeof declaredManifestRef !== "string" ||
    resolve(root, declaredManifestRef.replace(/^\//, "")) !==
      extractionManifestPath
  )
    fail(
      "Successful highlight requires extractionManifestUrl beside its POI tileset",
    );
  if (!existsSync(extractionManifestPath))
    fail(
      `Successful highlight extraction manifest does not exist: ${relative(root, extractionManifestPath)}`,
    );
  const extractionManifest = readJson(extractionManifestPath);
  if (
    extractionManifest.poiId !== result.result?.poiId ||
    !Array.isArray(extractionManifest.tiles) ||
    !extractionManifest.tiles.length
  )
    fail(
      "Successful highlight requires a non-empty extraction manifest for the same poiId",
    );
  const contentRefs = [];
  const visit = (tile) => {
  ...
  if (!contentRefs.length)
    fail(
      `Successful highlight tileset contains no tile content: ${tilesetRef}`,
    );
  for (const contentRef of contentRefs) {
    if (
      typeof contentRef !== "string" ||
      /^(?:https?:|data:)/i.test(contentRef)
    )
      fail(
        `Highlight tileset must reference local tile content: ${contentRef}`,
      );
    const contentPath = resolve(dirname(tilesetPath), contentRef);
    if (
      !contentPath.startsWith(
        `${dirname(tilesetPath)}${process.platform === "win32" ? "\\" : "/"}`,
      ) ||
      !existsSync(contentPath)
    )
      fail(
        `Successful highlight tile does not exist: ${relative(root, contentPath)}`,
      );
  }
  const manifestedFiles = new Set(
    extractionManifest.tiles.map((tile) => tile.poiFile),
  );
  if (
    contentRefs.some((contentRef) => !manifestedFiles.has(basename(contentRef)))
  )
    fail(
      "Highlight tileset content is not fully covered by its extraction manifest",
    );
  for (const backgroundRef of result.result?.backgroundTileRefs ?? []) {
    if (!existsSync(resolve(root, String(backgroundRef).replace(/^\//, ""))))
      fail(
        `Successful highlight background reference does not exist: ${backgroundRef}`,
      );
  }
}

function validateResolveRecoveryEvidence(
  runDir,
  runId,
  venueId,
  expectedEventIds,
  ...
  if (!["needs_review", "not_mappable"].includes(domain.resolutionStatus))
    fail(
      "Unresolved stage requires resolutionStatus needs_review or not_mappable",
    );
  if (
    !Array.isArray(domain.inputEventIds) ||
    expectedEventIds.some((id) => !domain.inputEventIds.includes(id))
  )
    fail("Resolution outcome must cover every venue-branch event");
  if (
    !domain.finalReason ||
    !Array.isArray(domain.evidenceInspected) ||
    domain.evidenceInspected.length === 0
  )
    fail("Resolution outcome requires evidence and a final reason");
  const isSearchHost = (hostname) =>
    /(?:^|\.)(?:google\.[a-z.]+|bing\.com|duckduckgo\.com|search\.yahoo\.com)$/i.test(
      hostname,
    );
  for (const evidence of domain.evidenceInspected) {
    ...
    try {
      ...
      fail(`Invalid resolution evidence URL: ${evidence.url}`);
    }
    if (isSearchHost(parsed.hostname))
      fail(
        "Search-result URLs are discovery aids, not inspected venue evidence",
      );
  }
  if (!domain.cacheKey || !domain.evidenceHash)
    fail("Resolution outcome requires reusable cacheKey and evidenceHash");
  if (domain.resolutionStatus === "not_mappable") {
    ...
    if (!classification || !allowedReasons.has(classification.reasonCode))
      fail(
        "not_mappable requires affirmative classification evidence; a local lookup miss is not sufficient",
      );
    if (
      !Array.isArray(classification.sourceUrls) ||
      classification.sourceUrls.length === 0
    )
      fail("not_mappable requires at least one evidence URL");
    for (const url of classification.sourceUrls) {
      try {
        const parsed = new URL(url);
        if (!["http:", "https:"].includes(parsed.protocol)) throw new Error();
      } catch {
        fail(`Invalid not_mappable evidence URL: ${url}`);
      }
    }
    return;
  }
  ...
  if (!Array.isArray(webResearch) || webResearch.length < 2)
    fail("needs_review requires at least two documented web research attempts");
  const researchTypes = new Set();
  for (const attempt of webResearch) {
    if (!["venue_official", "host_or_authority"].includes(attempt?.sourceType))
      fail(
        "Web research must cover venue_official and host_or_authority sources",
      );
    researchTypes.add(attempt.sourceType);
    if (typeof attempt.query !== "string" || !attempt.query.trim())
      fail("Each web research attempt requires its query");
    if (!attempt.checkedAt || !Number.isFinite(Date.parse(attempt.checkedAt)))
      fail("Each web research attempt requires a valid checkedAt timestamp");
    if (typeof attempt.outcome !== "string" || !attempt.outcome.trim())
      fail("Each web research attempt requires an outcome");
    if (!Array.isArray(attempt.resultUrls))
      fail(
        "Each web research attempt requires resultUrls, including an empty array when nothing was found",
      );
    for (const url of attempt.resultUrls) {
      ...
      if (isSearchHost(parsed.hostname))
        fail(
          "Web research resultUrls must be pages actually inspected, not search-result URLs",
        );
    }
  }
  if (
    !researchTypes.has("venue_official") ||
    !researchTypes.has("host_or_authority")
  )
    fail(
      "Web research must cover venue official and host/authority recovery paths",
    );

  const localLookups = domain.localLookupEvidence;
  if (!Array.isArray(localLookups) || localLookups.length < 2)
    fail(
      "needs_review requires address enrichment and OneMap candidate lookup evidence",
    );
  const lookupTools = new Set(localLookups.map((lookup) => lookup?.tool));
  if (
    ![...lookupTools].some((tool) =>
      [
        "venue-index:enrich",
        "venue-index:resolve",
        "search-local-venues",
    ...
    fail("needs_review requires a local address or coordinate lookup");
  if (!lookupTools.has("find-poi-tile-candidates"))
    fail("needs_review requires a clean OneMap candidate lookup");
  for (const lookup of localLookups) {
    if (
      typeof lookup.query !== "string" ||
      !lookup.query.trim() ||
      typeof lookup.outcome !== "string" ||
      !lookup.outcome.trim()
    )
      fail("Each local lookup requires a query and outcome");
  }

  if (
    !Array.isArray(domain.recoveryAttempts) ||
    domain.recoveryAttempts.length < 2
  )
    fail("needs_review requires two distinct recovery attempts");
  const attemptNumbers = new Set();
  for (const attempt of domain.recoveryAttempts) {
    if (
      !Number.isInteger(attempt?.attempt) ||
      attempt.attempt < 1 ||
      typeof attempt.approach !== "string" ||
      !attempt.approach.trim() ||
      typeof attempt.outcome !== "string" ||
      !attempt.outcome.trim()
    )
      fail(
        "Each recovery attempt requires an attempt number, approach, and outcome",
      );
    attemptNumbers.add(attempt.attempt);
  }
  if (!attemptNumbers.has(1) || !attemptNumbers.has(2))
    fail("needs_review requires recovery attempts 1 and 2");
  if (!Array.isArray(domain.competingCandidates))
    fail(
      "needs_review requires competingCandidates, including an empty array when no candidate exists",
    );
  const localResolutionPath = join(runDir, "local-venue-resolution.json");
  const localRow = existsSync(localResolutionPath)
    ? readJson(localResolutionPath).results?.find(
        (row) => row.venueId === venueId,
      )
    : null;
  const expectedGmlIds = [
    ...new Set(
      (localRow?.alternatives ?? []).flatMap(
        (candidate) => candidate.gmlIds ?? [],
      ),
    ),
  ];
  if (expectedGmlIds.length) {
    const submittedGmlIds = new Set(
      domain.competingCandidates.flatMap(
        (candidate) =>
          candidate?.gmlIds ?? (candidate?.gmlId ? [candidate.gmlId] : []),
      ),
    );
    if (expectedGmlIds.some((gmlId) => !submittedGmlIds.has(gmlId)))
      fail(
        "needs_review must explicitly carry every local OneMap alternative into competingCandidates",
      );
  }
}
...
function validateApprovedResolution(result, root = ROOT) {
  ...
  if (value.resolutionStatus !== "approved")
    fail("Successful resolve requires resolutionStatus approved");
  if (
    typeof value.poiId !== "string" ||
    !value.poiId ||
    typeof value.canonicalVenue !== "string" ||
    !value.canonicalVenue
  )
    fail("Approved resolution requires poiId and canonicalVenue");
  const gmlIds = value.gmlIds?.length
    ? value.gmlIds
    : [value.gmlId].filter(Boolean);
  if (
    !gmlIds.length ||
    gmlIds.some((gmlId) => typeof gmlId !== "string" || !gmlId) ||
    !Array.isArray(value.acceptedGmlNames) ||
    value.acceptedGmlNames.length === 0
  )
    fail(
      "Approved resolution requires exact GML identities and accepted names",
    );
  if (
    !Number.isFinite(value.coordinates?.lng) ||
    !Number.isFinite(value.coordinates?.lat)
  )
    fail("Approved resolution requires verified coordinates");
  if (!Array.isArray(value.sourceTiles) || value.sourceTiles.length === 0)
    fail("Approved resolution requires clean source tiles and batch IDs");
  for (const tile of value.sourceTiles) {
    const path = tile.path ?? tile.tilePath;
    if (
      typeof path !== "string" ||
      !/^(?:tiles|optimized-tiles)\/.+\.b3dm$/.test(path)
    )
      fail(
        "Approved resolution source tiles must reference clean tiles or optimized-tiles b3dm files",
      );
    if (
      !Array.isArray(tile.batchIds) ||
      tile.batchIds.length === 0 ||
      tile.batchIds.some((id) => !Number.isInteger(id) || id < 0)
    )
      fail(
        "Approved resolution source tiles require exact non-negative batch IDs",
      );
  }
  validateOneMapTileEvidence(root, value);
  if (!Array.isArray(value.evidence) || value.evidence.length === 0)
    fail("Approved resolution requires executable or authoritative evidence");
}
  ...
  if (result.sourceRole === "discovery") {
    for (const ref of result.artifactRefs)
      if (!existsSync(artifactPath(runDir, ref)))
        fail(`Discovery evidence does not exist: ${ref}`);
    for (const recordRef of result.processedSourceRecordRefs) {
      const base = baseArtifactRef(recordRef),
        envelope = readJson(artifactPath(runDir, base));
      const index = Number(recordRef.match(/#\/records\/(\d+)$/)?.[1]);
      if (envelope.records?.[index]?.recordType !== "discovery")
        fail(`Discovery record pointer does not resolve: ${recordRef}`);
    }
    return;
  }
  const totalEvidence = result.completion.providerTotalEvidence;
  if (result.completion.providerReportedTotal !== null) {
    if (!totalEvidence?.artifactRef || !totalEvidence?.jsonPointer)
      fail(
        "A successful API source requires providerTotalEvidence artifactRef and jsonPointer",
      );
    if (
      !/^raw\/[^/]+\/listings\/page-\d{4}\.json$/.test(
        totalEvidence.artifactRef,
      )
    )
      fail("providerTotalEvidence must target a raw listing JSON response");
    if (!declared.has(totalEvidence.artifactRef))
      fail("providerTotalEvidence artifact is not declared in artifactRefs");
    const providerTotal = jsonPointer(
      readJson(artifactPath(runDir, totalEvidence.artifactRef)),
      totalEvidence.jsonPointer,
    );
    if (providerTotal !== result.completion.providerReportedTotal)
      fail("providerReportedTotal does not match the raw provider response");
  }
  for (const pageRef of result.completion.pagesVisited) {
    if (!/^raw\/[^/]+\/listings\/page-\d{4}\.json$/.test(pageRef))
      fail(
        `API pagination evidence must be an untouched listing JSON response: ${pageRef}`,
      );
    if (!declared.has(pageRef))
      fail(`Pagination evidence is not declared in artifactRefs: ${pageRef}`);
    const pagePath = artifactPath(runDir, pageRef);
    if (!existsSync(pagePath))
      fail(`Pagination evidence does not exist: ${pageRef}`);
    const pageEvidence = readJson(pagePath);
    if (!pageEvidence || typeof pageEvidence !== "object")
      fail(`Pagination evidence is not a JSON object: ${pageRef}`);
  }
  const processedRefs =
    result.processedSourceRecordRefs ?? result.sourceRecordRefs;
  const invalidRefs = result.invalidSourceRecordRefs ?? [];
  const detailBases = new Set(processedRefs.map(baseArtifactRef));
  if (result.completion.detailPagesCaptured !== detailBases.size)
    fail("completion.detailPagesCaptured must match unique detail artifacts");
  if (
    !Number.isInteger(result.completion.detailUrlsDiscovered) ||
    result.completion.detailUrlsDiscovered < detailBases.size
  )
    fail(
      "completion.detailUrlsDiscovered must account for captured detail pages",
    );
  for (const recordRef of processedRefs) {
    const base = baseArtifactRef(recordRef);
    if (!/^raw\/[^/]+\/details\/[^/]+\.json$/.test(base))
      fail(
        `Source record pointer must target a detail fixture under raw/<source>/details/: ${recordRef}`,
      );
    if (!declared.has(base))
      fail(`Source record artifact is not declared in artifactRefs: ${base}`);
    ...
    if (
      ...
      fail(`Source record pointer does not resolve: ${recordRef}`);
    if (
      envelope.schemaVersion !== "1.0" ||
      envelope.runId !== state.runId ||
      !envelope.createdAt
    )
      fail(`Detail fixture envelope is not bound to the current run: ${base}`);
    const record = envelope.records[index];
    let detailUrl;
    try {
      detailUrl = new URL(record.detailUrl);
    } catch {
      fail(`Detail fixture has an invalid detailUrl: ${recordRef}`);
    }
    if (!["http:", "https:"].includes(detailUrl.protocol))
      fail(`Detail fixture has a non-web detailUrl: ${recordRef}`);
    const expectedFixtureName = `${sha(detailUrl.href)}.json`;
    if (basename(base) !== expectedFixtureName)
      fail(
        `Detail fixture filename mismatch for ${detailUrl.href}: expected ${expectedFixtureName}, received ${basename(base)}`,
      );
    if (
      !record.adapterId ||
      !record.adapterVersion ||
      !record.adapterDefinitionHash ||
      !record.providerId ||
      !record.providerOwner ||
      !["free", "open"].includes(record.providerCostClass) ||
      !record.retrievedAt ||
      !Number.isFinite(Date.parse(record.retrievedAt)) ||
      !Number.isInteger(record.listingPage) ||
      !record.sourceId
    ) {
      fail(
        `Detail fixture lacks required adapter/provider/retrieval identity: ${recordRef}`,
      );
    }
    if (
      run &&
      (record.requestedWindow?.start !== run.window.start ||
        record.requestedWindow?.end !== run.window.end ||
        record.requestedWindow?.timezone !== run.timezone)
    )
      fail(
        `Detail fixture requestedWindow does not match the run: ${recordRef}`,
      );
    const provenance = record.provenance;
    if (
      !provenance?.parentListingRef ||
      !provenance?.responseRef ||
      !provenance?.officialReferenceRef ||
      !provenance?.officialReference
    ) {
      fail(
        `Detail fixture lacks listing/detail/official provenance pointers: ${recordRef}`,
      );
    }
    for (const evidenceRef of [
      ...
      if (
        !declared.has(evidenceRef) ||
        !existsSync(artifactPath(runDir, evidenceRef))
      )
        fail(
          `Detail fixture provenance is missing declared evidence: ${evidenceRef}`,
        );
    }
    const official = readJson(
      artifactPath(runDir, provenance.officialReferenceRef),
    );
    if (
      official.status < 200 ||
      official.status >= 400 ||
      official.status !== provenance.officialReference.status ||
      official.finalUrl !== provenance.officialReference.finalUrl
    )
      fail(`Official event reference evidence is invalid: ${recordRef}`);
  }
  for (const recordRef of invalidRefs) {
    const base = baseArtifactRef(recordRef);
    if (!declared.has(base))
      fail(
        `Invalid source record artifact is not declared in artifactRefs: ${base}`,
      );
...
function validateSourceSemantics(runDir, state, result) {
  ...
  if (result.sourceRole === "discovery") {
    ...
    const outcomes = Object.entries(
      result.counts.confirmationOutcomeCounts ?? {},
    );
    if (outcomes.some(([, count]) => !Number.isInteger(count) || count < 0))
      fail("Discovery confirmation outcome counts are invalid");
    const occurrencesEmitted = outcomes.reduce(
      (sum, [, count]) => sum + count,
      0,
    );
    const eligiblePreDedup = outcomes.reduce(
      (sum, [decision, count]) =>
        sum + (publishableDecisions.has(decision) ? count : 0),
      0,
    );
    const excludedOccurrences = occurrencesEmitted - eligiblePreDedup;
    if (
      result.counts.discoveryRecordsReceived !==
        result.counts.sourceRecordsReceived ||
      result.counts.processedSourceRecords !==
        result.processedSourceRecordRefs.length ||
      ...
      fail(
        "Discovery source accounting does not match authority-confirmation outcomes",
      );
    }
    return;
  }
  ...
  for (const recordRef of result.processedSourceRecordRefs) {
    ...
    if (!["physical", "online", "hybrid", "unknown"].includes(record.mode))
      fail(`Processed source record has unsupported mode: ${recordRef}`);
    const occurrences =
      Array.isArray(record.performances) && record.performances.length
        ? record.performances
        : [record];
  ...
  if (result.counts.occurrencesEmitted !== occurrencesEmitted)
    fail(
      `occurrencesEmitted must equal ${occurrencesEmitted} from detail fixtures`,
    );
  if (result.counts.excludedOccurrences !== excludedOccurrences)
    fail(
      `excludedOccurrences must equal ${excludedOccurrences} from the run window and mode contract`,
    );
  if (result.counts.eligiblePreDedup !== eligiblePreDedup)
    fail(
      `eligiblePreDedup must equal ${eligiblePreDedup} from the run window and mode contract`,
    );
}
...
function recordSource(options) {
  const runDir = runDirectory(options.run);
  const state = loadState(options.run);
  if (!(options.source in state.sources))
    fail(`Unknown source: ${options.source}`);
  ...
  state.sources[options.source] = {
    ...
    operatingMode:
      result.operatingMode ?? state.sources[options.source].operatingMode,
    confirmationRefs: result.confirmationRefs ?? [],
    ...
    invalidReasonCodes: result.invalidReasonCodes ?? {},
...
async function collectSourceCommand(options) {
  ...
  const source = config.sources.find(
    (item) => item.enabled && item.name === options.source,
  );
  if (!source)
    fail(`No enabled executable adapter for source: ${options.source}`);
  const resultPath = join(runDir, `${source.adapterId}.result.json`);
  const corroborationRecords = Object.entries(state.sources).flatMap(
    ([sourceName, collected]) => {
      if (sourceName === options.source || collected.status !== "success")
        return [];
      return (collected.processedSourceRecordRefs ?? []).flatMap(
        (recordRef) => {
          try {
            const envelope = readJson(
              artifactPath(runDir, baseArtifactRef(recordRef)),
            );
            const index = Number(recordRef.match(/#\/records\/(\d+)$/)?.[1]);
            const record = envelope.records?.[index];
            return record
              ? [
                  {
                    ...
                    sourceRecordId:
                      ...
                      record.sourceId ??
                      record.discoveryRecordId,
                    sourceRole: collected.sourceRole,
                  },
                ]
              : [];
          } catch {
  ...
  const result = await collectSource({
    ...
    logger: (record) =>
      pipelineTrace(runDir, {
        stage: record.stage ?? "retrieval",
        action: record.action ?? "retrieval",
        outcome: record.action?.includes("complete") ? "success" : "started",
        sourceName: options.source,
        entityType: "request",
        entityId:
          record.entityId ?? `${options.source}:${record.pageIndex ?? "batch"}`,
  ...
  if (result.status === "success")
    updateLastSuccessfulUse(options.source, new Date().toISOString());
}
...
function recordNormalization(options) {
  ...
  if (result.status !== "success" && result.status !== "failed")
    fail("Normalization status must be success or failed");
  if (result.status === "success") {
    ...
    const normalizedExcluded = readJson(
    ...
    for (const [name, envelope] of [
      ...
      if (
        !Array.isArray(envelope.records) ||
        envelope.counts?.records !== envelope.records.length
      )
    ...
    for (const [sourceName, recordRefs] of Object.entries(
      ...
      if (!source || source.status !== "success" || !Array.isArray(recordRefs))
        fail(`Invalid source reclassification: ${sourceName}`);
      for (const recordRef of recordRefs) {
        ...
        if (
          ...
          !source.invalidSourceRecordRefs.includes(recordRef)
        ) {
          fail(
            `Source reclassification is not an optional-field record: ${sourceName}/${recordRef}`,
          );
        }
        source.invalidSourceRecordRefs = source.invalidSourceRecordRefs.filter(
          (ref) => ref !== recordRef,
        );
        if (!source.processedSourceRecordRefs.includes(recordRef))
          source.processedSourceRecordRefs.push(recordRef);
        delete source.invalidReasonCodes[recordRef];
      }
      source.counts.invalidSourceRecords =
        source.invalidSourceRecordRefs.length;
      source.counts.processedSourceRecords =
        source.processedSourceRecordRefs.length;
      if (
        source.counts.sourceRecordsReceived !==
        source.counts.invalidSourceRecords +
          source.counts.processedSourceRecords
      ) {
    ...
    if (
      normalizedInvalid.records.some(
        (record) => typeof record.reasonCode !== "string" || !record.reasonCode,
      )
    )
      fail("Every normalized invalid record requires a stable reasonCode");
    const actualInvalidRefs = new Set(
      normalizedInvalid.records.map((record) => record.sourceRecordRef),
    );
    if (
      actualInvalidRefs.size !== normalizedInvalid.records.length ||
      expectedInvalidRefs.size !== actualInvalidRefs.size ||
      [...expectedInvalidRefs].some((ref) => !actualInvalidRefs.has(ref))
    ) {
      fail(
        "normalized/invalid.json does not exactly account for invalid source record refs",
      );
    }
    const processedEvidenceRefs = new Set([
      ...normalizedEvents.records.flatMap((event) =>
    ...
    if (
      [...expectedProcessedRefs].some(
        (ref) => !processedEvidenceRefs.has(ref),
      ) ||
    ...
    for (const [sourceName, accounting] of Object.entries(
      ...
      if (
        ...
        fail(
          `Normalization source accounting does not reconcile for ${sourceName}`,
        );
    ...
    for (const key of [
      ...
      if (!Number.isInteger(result.counts?.[key]) || result.counts[key] < 0)
        fail(`Invalid normalization count: ${key}`);
    }
    if (result.counts.eligiblePreDedup !== eligiblePreDedup)
      fail(
    ...
    if (
      ...
      fail("Deduplication accounting does not reconcile");
    if (result.counts.acceptedPrimary !== result.counts.acceptedPostDedup)
      fail("Primary attribution accounting does not reconcile");
    if (!Array.isArray(result.venueBranches))
      fail("Successful normalization requires venueBranches");
    validateNormalizedSemantics(
      runDir,
      normalizedEvents,
      result,
      readJson(join(runDir, "run.json")).window,
    );
    const previousVenues = state.venues;
    const reconciled = reconcileNormalizedVenueBranches(
      previousVenues,
      result.venueBranches,
    );
    ...
    registerArtifacts(
      ...
      successfulSources.flatMap((source) =>
        source.artifactRefs
          .map(
            (reference) =>
              readJson(join(runDir, "run.json")).artifacts[reference]?.sha256,
          )
          .filter(Boolean),
      ),
    );
  ...
  if (result.status === "success" && result.venueBranches?.length === 0)
    state.resolutionPreparation = {
      status: "success",
      artifactRefs: [],
      error: null,
    };
  else if (normalizationChangedBranch)
    state.resolutionPreparation = {
      status: "pending",
      artifactRefs: [],
      error: null,
...
function finalizeDedupCommand(options) {
  const runDir = runDirectory(options.run),
    state = loadState(options.run);
  if (state.normalization?.status !== "success")
    fail("Cannot finalize duplicates before successful normalization");
  if (
    Object.values(state.venues).some(
      (venue) => !TERMINAL_STAGE.has(venue.stages.resolve.status),
    )
  )
    fail(
      "Cannot finalize duplicates before every venue resolution is terminal",
    );
  const eventsPath = join(runDir, "normalized/events.json"),
    envelope = readJson(eventsPath),
    originalEvents = envelope.records;
  const resolutions = {};
  for (const [venueId, venue] of Object.entries(state.venues)) {
    const resolveStage = venue.stages.resolve;
    resolutions[venueId] =
      resolveStage.status === "success" && resolveStage.outputRef
  ...
  const sourcePrecedence = Object.fromEntries(
    readPipelineConfig()
      ...
      .map(({ name, precedence }) => [name, precedence]),
  );
  ...
  const isolatedReviewIds = new Set(
    result.blockingReviews.flatMap(({ occurrenceIds = [] }) => occurrenceIds),
  );
  for (const event of result.events)
    if (
      (event.sourceOccurrenceIds ?? []).some((id) => isolatedReviewIds.has(id))
    ) {
      Object.assign(event, {
        lifecycleState: "held",
        ...
        reviewStatus: "review",
        reviewReason: "prior_cluster_join_review",
      });
    }
  const candidatesRef = "normalized/dedup-candidates.json",
    decisionsRef = "normalized/dedup-final-decisions.json";
  writeJson(join(runDir, candidatesRef), {
    schemaVersion: "1.0",
    runId: options.run,
    counts: { records: candidates.length },
    records: candidates,
  });
  writeJson(join(runDir, decisionsRef), {
    schemaVersion: "1.0",
    runId: options.run,
    counts: { records: result.decisions.length },
    records: result.decisions,
  });
  writeJson(eventsPath, {
    ...envelope,
        ...
        venue.eventIds.map(
          (id) =>
            result.events.find((event) =>
              event.sourceOccurrenceIds.includes(id),
            )?.id ?? id,
        ),
      ),
    ];
  ...
  pipelineTrace(runDir, {
    stage: "deduplication",
    action: "deduplication_terminal",
    outcome: state.deduplication.status,
    entityType: "run",
    entityId: options.run,
    counts: result.counts,
    reasonCode: result.blockingReviews.length
      ? "prior_cluster_join_review"
      : null,
    blocker: state.deduplication.error,
    nextAction: `npm run event-pipeline -- advance --run ${options.run}`,
  });
  printNext(state);
}
...
function prepareVenues(options) {
  ...
  const state = loadState(options.run);
  if (state.normalization.status !== "success")
    fail("Cannot prepare venue recovery before successful normalization");
  ...
  if (existsSync(indexPath)) {
    try {
      ...
      hasOneMapRows =
        db
          .prepare(
            "SELECT count(*) AS count FROM places WHERE source='onemap-3d'",
          )
          .get().count > 0;
      db.close();
    } catch {
  ...
  if (!hasOneMapRows)
    execFileSync("npm", ["run", "venue-index:build"], {
      cwd: ROOT,
      stdio: "inherit",
    });
  try {
    execFileSync(
      "npm",
      ["run", "venue-index:enrich", "--", "--run", options.run],
    ...
    for (const venue of Object.values(refreshedState.venues)) {
      if (
        venue.stages.resolve.status !== "unresolved" ||
        !venue.stages.resolve.outputRef
      )
        continue;
      const previous = readJson(join(runDir, venue.stages.resolve.outputRef));
      if (previous.result?.evidenceHash === branchEvidenceHash(runDir, venue))
        continue;
      if (run.artifacts[venue.stages.resolve.outputRef])
        run.artifacts[venue.stages.resolve.outputRef].status = "invalidated";
      for (const stage of STAGES)
        venue.stages[stage] = {
          status: "pending",
          outputRef: null,
    ...
    execFileSync(
      "npm",
      ["run", "venue-index:resolve", "--", "--run", options.run],
      { cwd: ROOT, stdio: "inherit" },
    );
  } catch (error) {
    state.resolutionPreparation = {
      status: "failed",
      artifactRefs: [],
      error: `Venue recovery preparation failed with exit code ${error.status ?? 1}`,
    };
    saveState(runDir, state);
    throw error;
  }
  const refs = [
    "normalized/location-enrichment.json",
    "local-venue-resolution.json",
    "local-venue-resolution.md",
  ];
  registerArtifacts(runDir, refs);
  const localCandidateCount = readJson(
    join(runDir, "local-venue-resolution.json"),
  ).results.filter((row) => row.status === "candidate_matched").length;
  const preparedState = loadState(options.run);
  preparedState.resolutionPreparation = {
    status: "success",
    artifactRefs: refs,
    localCandidateCount,
    error: null,
...
function recordStageValue(options, result, resultPath) {
  ...
  if (
    ...
    fail(
      "Cannot record resolver outcomes before prepare-venues completes enrichment and local recovery",
    );
  if (
    stageIndex > 0 &&
    venue.stages[STAGES[stageIndex - 1]].status !== "success"
  )
    fail(
      `Cannot run ${options.stage} before successful ${STAGES[stageIndex - 1]}`,
    );
  if (!TERMINAL_STAGE.has(result.status) || result.status === "pending")
    fail("Stage result is not terminal");
  if (result.stage !== options.stage)
    fail("Stage result does not match --stage");
  if (
    result.status === "success" &&
    (!Array.isArray(result.outputRefs) || result.outputRefs.length === 0) &&
    result.result?.changeAction !== "noop"
  )
    fail("Successful changed stage requires outputRefs");
  if (
    result.status === "success" &&
    ["highlight", "pill", "panel"].includes(options.stage) &&
    !["create", "update", "noop"].includes(result.result?.changeAction)
  )
    fail("Frontend stage requires changeAction create, update, or noop");
  if (result.status === "success")
    validateStageEventIds(options.stage, venue.eventIds, result);
  validateApprovedResolution(result);
  if (options.stage === "resolve" && result.status === "unresolved") {
    const expectedHash = branchEvidenceHash(runDir, venue),
      expectedKey = normalizeText(venue.venue);
    if (
      result.result?.evidenceHash !== expectedHash ||
      result.result?.cacheKey !== expectedKey
    )
      fail(
        "Unresolved resolution cacheKey or evidenceHash does not match executable branch evidence",
      );
    if (
      result.result?.resolutionStatus === "needs_review" &&
      !options.allowCachedUnresolved
    ) {
      const records = recoveryRecords(runDir),
        index = records.findIndex((record) => record.venueId === options.venue);
      const expectedRef = `normalized/venue-recovery-evidence.json#/records/${index}`;
      if (index < 0 || result.result?.recoveryEvidenceRef !== expectedRef)
        fail(
          "needs_review requires an applied venue-recovery evidence checkpoint before it can be recorded",
        );
    }
  }
  ...
  if (result.status !== "success" && !result.error)
    fail("Non-success stage requires an error");
  ...
  venue.stages[options.stage] = {
    ...
    error: result.error ?? null,
    ...(options.stage === "resolve"
      ? { resolutionStatus: result.result?.resolutionStatus ?? null }
      : {}),
  };
  if (options.stage === "resolve" && result.status === "success") {
    const registry = readJson(aliasRegistryPath()),
      value = result.result;
    const entry = {
      status: "approved",
      rawVenue: venue.venue,
      normalizedVenue: normalizeText(venue.venue),
      canonicalVenue: value.canonicalVenue,
      verifiedAddress: value.verifiedAddress ?? null,
      coordinates: value.coordinates,
      poiId: value.poiId,
      gmlId: value.gmlId ?? null,
      gmlIds: value.gmlIds ?? (value.gmlId ? [value.gmlId] : []),
      acceptedGmlNames: value.acceptedGmlNames,
      sourceTiles: value.sourceTiles,
      evidence: value.evidence,
      verifiedAt: new Date().toISOString(),
    };
    registry.entries = [
      ...registry.entries.filter(
        (item) => normalizeText(item.rawVenue) !== entry.normalizedVenue,
      ),
      entry,
    ];
  ...
  if (options.stage === "resolve" && result.status === "unresolved") {
    ...
    const entry = {
      ...
      status: result.result.resolutionStatus,
      result: result.result,
      updatedAt: new Date().toISOString(),
    };
    cache.entries = [
      ...cache.entries.filter((item) => item.cacheKey !== entry.cacheKey),
      entry,
    ];
  ...
  if (result.status !== "success") {
    for (const downstream of STAGES.slice(stageIndex + 1))
      venue.stages[downstream] = {
        status: "skipped",
        outputRef: null,
        error: `Upstream ${options.stage} was ${result.status}`,
      };
  }
  saveState(runDir, state);
  pipelineTrace(runDir, {
    stage: options.stage,
    action: "venue_stage_terminal",
    outcome: result.status,
    entityType: "venue",
    entityId: options.venue,
    counts: { events: venue.eventIds.length },
    reasonCode: result.result?.resolutionStatus ?? null,
    blocker: result.error ?? null,
    nextAction: `npm run event-pipeline -- advance --run ${options.run}`,
  });
...
function recordStage(options) {
  return recordStageValue(
    options,
    readResult(options.result),
    resolve(ROOT, options.result ?? ""),
  );
}
...
function recordRecoveredUnresolved(runId, venueId, venue, recovery, localRow) {
  ...
  const result = {
    ...
    startedAt: now,
    endedAt: now,
    inputRefs: venue.eventIds,
    outputRefs: [],
    error: finalReason,
    nextStep: null,
    result: {
      resolutionStatus,
      inputVenue: venue.venue,
      inputEventIds: venue.eventIds,
      finalReason,
      cacheKey: normalizeText(venue.venue),
      evidenceHash: branchEvidenceHash(runDir, venue),
      recoveryEvidenceRef: recoveryEvidenceRef(runDir, venueId),
      evidenceInspected: recovery.evidenceInspected.map(
        ({ sourceType, label, url }) => ({ sourceType, label, url }),
      ),
      ...(classification
        ? { notMappableEvidence: classification }
        : {
            webResearch: recovery.evidenceInspected.map(
              ({ sourceType, query, checkedAt, outcome, url }) => ({
                sourceType,
                query,
                checkedAt,
                outcome,
            ...
            localLookupEvidence: [
              {
                ...
                query: [
                  ...(recovery.addressCandidates ?? []),
                  ...(recovery.postalCodes ?? []),
                  venue.venue,
                ].join(" | "),
                outcome:
                  localRow?.reason ??
                  "No local place or fixed building matched",
              },
              {
                tool: "find-poi-tile-candidates",
                query: venue.venue,
                outcome: alternatives.length
                  ? `${alternatives.length} executable OneMap candidates remained ambiguous`
                  : "No executable OneMap candidate was returned",
              },
            ],
            recoveryAttempts: recovery.evidenceInspected
              .slice(0, 2)
              .map((evidence, index) => ({
                attempt: index + 1,
                approach: evidence.query,
                outcome: evidence.outcome,
              })),
            competingCandidates: alternatives.map((candidate) => ({
              key: candidate.key,
              name: candidate.name,
              gmlIds: candidate.gmlIds ?? [],
  ...
  writeJson(resultPath, result);
  return recordStageValue(
    { run: runId, venue: venueId, stage: "resolve", quiet: true },
    result,
    resultPath,
  );
}

async function applyDeterministicAddressRecovery(
  runId,
  venueId,
  venue,
  localRow,
  ...
  const runDir = runDirectory(runId);
  if (
    deterministicRecoveryRecords(runDir).some(
      (record) => record.venueId === venueId,
    )
  )
    return false;
  ...
  const location = {
    addressCandidates: [
      ...new Set([
        ...(localRow?.locationEvidence?.addressCandidates ?? []),
        ...saved.addressCandidates,
      ]),
    ],
    postalCodes: [
      ...new Set([
        ...(localRow?.locationEvidence?.postalCodes ?? []),
        ...saved.postalCodes,
      ]),
    ],
    coordinateCandidates:
      localRow?.locationEvidence?.coordinateCandidates ?? [],
  };
  if (
    (location.coordinateCandidates ?? []).length ||
    !(location.addressCandidates ?? []).length
  )
    return false;
  let selected = null,
    queryResult = null,
  ...
  for (const address of location.addressCandidates) {
    let response;
    try {
      response = await queryOneMap(address);
    } catch {
      continue;
    }
  ...
  envelope.records = [
    ...(envelope.records ?? []).filter((record) => record.venueId !== venueId),
    {
      venueId,
      venue: venue.venue,
      method: "exact_onemap_address_geocode",
      verifiedAddress,
      addressCandidates: location.addressCandidates ?? [],
      ...
      coordinateCandidates: [
        {
          ...
          lng: selected.longitude,
          source: "onemap_public_exact_address",
          recordRef: queryResult.requestUrl,
          evidenceField: queryResult.selectedQuery,
        },
      ],
      result: selected,
  ...
  writeJson(path, envelope);
  registerArtifacts(runDir, [
    "normalized/deterministic-location-recovery.json",
  ]);
  const child = spawnSync(
    process.execPath,
    [
      join(ROOT, "scripts/resolve-venues-locally.mjs"),
      "--run",
      runId,
      "--ids",
      venueId,
  ...
  if (child.status !== 0)
    fail(
      `Focused deterministic address recovery failed: ${child.stderr || child.stdout}`,
    );
  registerArtifacts(runDir, [
    "local-venue-resolution.json",
    "local-venue-resolution.md",
  ...
  state.resolutionPreparation = {
    ...state.resolutionPreparation,
    localCandidateCount: local.results.filter(
      (row) =>
        row.status === "candidate_matched" &&
        state.venues[row.venueId]?.stages.resolve.status === "pending",
    ).length,
  };
  saveState(runDir, state);
  return true;
...
async function recordVenueRecovery(options) {
  const runDir = runDirectory(options.run),
    state = loadState(options.run),
    venue = state.venues[options.venue];
  if (!venue) fail(`Unknown venue branch: ${options.venue}`);
  if (!options.evidence)
    fail("record-venue-recovery requires --evidence <json>");
  if (state.resolutionPreparation?.status !== "success")
    fail("Venue recovery can only be recorded after prepare-venues");
  if (venue.stages.resolve.status === "success" && !options.replace)
    fail(
      "Cannot replace an already approved venue with recovery evidence without --replace",
    );
  const submitted = validateVenueRecoveryEvidence(
    readJson(resolve(ROOT, options.evidence)),
    venue.venue,
  );
  const normalized = await enrichRecoveryCoordinates(
    classifyNonBuildingRecovery(submitted),
  );
  const previousLocal = existsSync(join(runDir, "local-venue-resolution.json"))
    ? (readJson(join(runDir, "local-venue-resolution.json")).results?.find(
        (row) => row.venueId === options.venue,
      ) ?? null)
    : null;
  ...
  const previousIndex = envelope.records.findIndex(
    (item) => item.venueId === options.venue,
  );
  if (previousIndex >= 0) envelope.records[previousIndex] = record;
  else envelope.records.push(record);
  envelope.updatedAt = new Date().toISOString();
  ...
  if (
    venue.stages.resolve.outputRef &&
    run.artifacts[venue.stages.resolve.outputRef]
  )
    run.artifacts[venue.stages.resolve.outputRef].status = "invalidated";
  for (const stage of STAGES)
    venue.stages[stage] = { status: "pending", outputRef: null, error: null };
  state.verification = {
    status: "pending",
    build: null,
    eventUi: null,
  ...
  if (record.notMappableEvidence) {
    const terminalState = recordRecoveredUnresolved(
      options.run,
      options.venue,
      venue,
      record,
    ...
    if (!options.quiet)
      process.stdout.write(
        `${JSON.stringify(
          {
            ...progressResponse(terminalState),
            recoveryEvidenceRef: recoveryEvidenceRef(runDir, options.venue),
            localEvidence: compactLocalEvidence(previousLocal),
            recordedUnresolved: true,
          },
          null,
          2,
        )}\n`,
  ...
  if (child.status !== 0)
    fail(
      `Focused local venue recovery failed: ${child.stderr || child.stdout}`,
    );
  registerArtifacts(runDir, [
    "local-venue-resolution.json",
    "local-venue-resolution.md",
  ...
  refreshed.resolutionPreparation = {
    ...refreshed.resolutionPreparation,
    localCandidateCount: local.results.filter(
      (row) =>
        row.status === "candidate_matched" &&
        refreshed.venues[row.venueId]?.stages.resolve.status === "pending",
    ).length,
  };
  saveState(runDir, refreshed);
  const localEvidence =
    local.results.find((row) => row.venueId === options.venue) ?? null;
  const terminalState =
    localEvidence?.status === "candidate_matched"
      ? refreshed
      : recordRecoveredUnresolved(
          options.run,
          options.venue,
          refreshed.venues[options.venue],
          record,
          localEvidence,
        );
  if (!options.quiet)
      ...
      `${JSON.stringify(
        {
          ...progressResponse(terminalState),
          recoveryEvidenceRef: recoveryEvidenceRef(runDir, options.venue),
          localEvidence: compactLocalEvidence(localEvidence),
          recordedUnresolved: localEvidence?.status !== "candidate_matched",
        },
        null,
        2,
      )}\n`,
...
function resolveLocal(options) {
  ...
  for (const row of local.results.filter(
    (item) =>
      item.status === "candidate_matched" &&
      stateBefore.venues[item.venueId]?.stages.resolve.status === "pending",
  )) {
    const building = row.building;
    const complete =
      building?.gmlIds?.length > 0 &&
      building?.sourceTiles?.length > 0 &&
      building.sourceTiles.every(
        (tile) => tile.tilePath && tile.batchIds?.length,
      );
    if (!complete) {
    ...
    const poiId = String(row.alias?.canonicalVenue ?? building.name)
      .toLowerCase()
      .normalize("NFKD")
      .replace(/[\u0300-\u036f]/g, "")
      .replace(/[^a-z0-9]+/g, "-")
    ...
    const result = {
      ...
      startedAt: timestamp,
      endedAt: timestamp,
      inputRefs: row.eventIds,
      outputRefs: [`stages/${row.venueId}/resolve.json`],
      error: null,
      nextStep: null,
      result: {
        resolutionStatus: "approved",
        inputVenue: row.venue,
        canonicalVenue: row.alias?.canonicalVenue ?? building.name,
        verifiedAddress:
          row.place?.address ??
          row.locationEvidence?.addressCandidates?.[0] ??
          null,
        poiId,
        gmlId: building.gmlIds?.length === 1 ? building.gmlIds[0] : null,
        gmlIds: building.gmlIds,
        acceptedGmlNames: row.alias?.acceptedGmlNames ??
          building.acceptedGmlNames ?? [building.name],
        coordinates: { lng: building.longitude, lat: building.latitude },
        sourceTiles: (building.sourceTiles ?? []).map(
          ({ tilePath, batchIds }) => ({ path: tilePath, batchIds }),
        ),
        evidence: [
          {
            type: building.adminReview
              ? "admin_selection_revalidated"
              : row.alias
                ? "approved_alias_local_index"
                : "executable_local_resolution",
            detail: row.reason,
            verifiedAt: local.generatedAt,
            ...(building.adminReview
              ? {
                  reviewId: building.adminReview.reviewId,
                  evidenceHash: building.adminReview.evidenceHash,
                  candidateGmlId: building.adminReview.candidateGmlId,
                  decisionReason: building.adminReview.decisionReason,
                  decidedAt: building.adminReview.decidedAt,
                }
              : {}),
          },
        ],
    ...
    writeJson(resultPath, result);
    recordStageValue(
      { run: options.run, venue: row.venueId, stage: "resolve", quiet: true },
      result,
      resultPath,
    );
    accepted += 1;
  ...
  saveState(runDir, state);
  process.stdout.write(
    `${JSON.stringify({ ...progressResponse(state), locallyApproved: accepted }, null, 2)}\n`,
  );
  if (!state.finalizedAt) process.exitCode = CONTINUE_EXIT_CODE;
}
...
function reprocessUnresolved(options) {
  ...
  if (child.status !== 0)
    fail(
      `Unresolved-only local venue rerun failed: ${child.stderr || child.stdout}`,
    );
  registerArtifacts(runDir, [
    "local-venue-resolution.json",
    "local-venue-resolution.md",
  ...
  saveState(runDir, state);
  process.stdout.write(
    `${JSON.stringify({ ...progressResponse(state), reopenedVenueIds, unresolvedChecked: local.results.length }, null, 2)}\n`,
  );
  if (!state.finalizedAt) process.exitCode = CONTINUE_EXIT_CODE;
}
...
function branchEvidenceHash(runDir, venue) {
  ...
  const enrichment = existsSync(enrichmentPath)
    ? (readJson(enrichmentPath).records ?? [])
    : [];
...
function reusableResolutionEntry(
  cache,
  normalizedVenue,
  evidenceHash,
  eventIds = [],
  ...
  const coversCurrentCandidates = (entry) => {
    if (
      entry.result?.resolutionStatus !== "needs_review" ||
      !expectedGmlIds.length
    )
      return true;
    const cached = new Set(
      (entry.result.competingCandidates ?? []).flatMap(
        (candidate) =>
          candidate?.gmlIds ?? (candidate?.gmlId ? [candidate.gmlId] : []),
      ),
    );
  ...
  const entries =
    cache.entries?.filter(
      (entry) =>
        entry.normalizedVenue === normalizedVenue &&
        coversCurrentCandidates(entry),
    ) ?? [];
  const sameEvents = (entry) => {
    const cached = entry.result?.inputEventIds;
    return (
      Array.isArray(cached) &&
      cached.length === eventIds.length &&
      cached.every((id) => eventIds.includes(id))
    );
  };
  return (
    entries.find((entry) => entry.evidenceHash === evidenceHash) ??
    entries.find(sameEvents) ??
    entries.find((entry) =>
      ["mobile_venue", "multi_venue"].includes(
        entry.result?.notMappableEvidence?.reasonCode,
      ),
    ) ??
    null
  );
...
function reuseCachedResolutions(runId) {
  const runDir = runDirectory(runId),
    ...
    cache = readResolutionCache();
  const localResolutionPath = join(runDir, "local-venue-resolution.json");
  const localRows = new Map(
    (existsSync(localResolutionPath)
      ? (readJson(localResolutionPath).results ?? [])
      : []
    ).map((row) => [row.venueId, row]),
  );
  let reused = 0;
  for (const [venueId, venue] of Object.entries(state.venues)) {
    ...
    const expectedGmlIds = [
      ...new Set(
        (localRows.get(venueId)?.alternatives ?? []).flatMap(
          (candidate) => candidate.gmlIds ?? [],
        ),
      ),
    ];
    const entry = reusableResolutionEntry(
      cache,
    ...
    const result = {
      ...
      inputRefs: venue.eventIds,
      outputRefs: [],
      error: entry.result.finalReason,
      nextStep: null,
      result: {
        ...entry.result,
        inputEventIds: venue.eventIds,
    ...
    const resultPath = join(runDir, `resolve-cache-${venueId}.json`);
    writeJson(resultPath, result);
    recordStageValue(
      {
        run: runId,
...
function reuseResolutionCacheCommand(options) {
  const reused = reuseCachedResolutions(options.run),
    state = loadState(options.run);
  process.stdout.write(
    `${JSON.stringify({ ...progressResponse(state), reused }, null, 2)}\n`,
  );
  if (!state.finalizedAt) process.exitCode = CONTINUE_EXIT_CODE;
}
...
function collectLocationClues(value, output = []) {
  if (output.length >= 8 || value === null || value === undefined)
    return output;
  if (typeof value === "string") {
    ...
    const match =
      /\baddress\b|\bSingapore\s+\d{6}\b|\b\d{1,4}[A-Za-z]?\s+[A-Za-z][^,.]{1,60}\b(?:Road|Street|Avenue|Lane|Drive|Crescent|Walk|Boulevard|Terrace|Place|Parkway)\b/i.exec(
        text,
      );
    if (match)
      output.push(
        text.slice(
          ...
          Math.min(text.length, match.index + 520),
        ),
      );
    return output;
  }
...
function branchSavedAddressEvidence(runDir, venue) {
  ...
  const values = selected.flatMap((event) =>
    [event.venue, event.address, event.description].filter(Boolean),
  );
  for (const source of selected.flatMap((event) => event.sources ?? [])) {
    ...
    for (const payloadRef of [
      baseRef,
      baseRef.replace(/\.json$/, ".response.json"),
    ]) {
      if (!existsSync(artifactPath(runDir, payloadRef))) continue;
      try {
        collectLocationStrings(
          readJson(artifactPath(runDir, payloadRef)),
          values,
        );
      } catch {
        /* retain other saved evidence */
      }
    }
  }
...
function collectOfficialCandidatePages(value, output = [], path = []) {
  ...
  for (const [key, item] of Object.entries(value)) {
    ...
    if (
      typeof item === "string" &&
      /^(?:BookingUrl|EventWebsite|Website|ExternalLink|OrganizerUrl|VenueUrl)$/i.test(
        key,
      )
    ) {
      try {
        ...
        if (
          ...
          !output.some((entry) => entry.url === url.href)
        ) {
...
function venueRecoveryContext(runId, venueId, venue, localRow) {
  ...
  const rawLocationClues = [],
    officialCandidatePages = [];
  for (const source of selected.flatMap((event) => event.sources ?? [])) {
    if (!source.recordRef) continue;
    const baseRef = baseArtifactRef(source.recordRef);
    ...
    for (const payloadRef of payloadRefs) {
      ...
      try {
        ...
        const raw =
          payloadRef === baseRef
            ? jsonPointer(payload, pointerFromArtifactRef(source.recordRef))
            : payload;
        for (const clue of collectLocationClues(raw))
          if (!rawLocationClues.includes(clue)) rawLocationClues.push(clue);
        for (const page of collectOfficialCandidatePages(raw))
          if (!officialCandidatePages.some((entry) => entry.url === page.url))
            officialCandidatePages.push(page);
      } catch {
  ...
  const notMappableContract = {
    ...
    sourceUrlsFormat:
      'Array of HTTP(S) URL strings. Example: ["https://venue.example/event"]. Labelled objects are accepted and normalized, but strings are canonical.',
    selectionRule:
      "Use a reason only after authoritative evidence proves that no single stable OneMap building can represent the event. If a route or mobile experience has a fixed building start, pickup, or meeting point, resolve that building instead.",
    reasons: {
      outside_singapore: "The verified event location is outside Singapore.",
      mobile_venue:
        "The event itself moves or follows a route and authoritative evidence provides no single stable building start, pickup, or meeting point.",
      multi_venue:
        "The event spans multiple physical venues and authoritative evidence provides no single primary building anchor.",
      no_target_building:
        "The verified fixed event location has no exact OneMap building GML, such as an MRT platform, gantry, passage, or standalone exit.",
    },
  };
  const recoveryFieldFormats = {
    ...
    evidenceInspected: [
      {
        sourceType:
          "venue_official for the actual venue/operator site; host_or_authority for supplied Catch/SISTIC event pages",
        label: "Opened page label",
        url: "https://actual-inspected-page.example/path",
        query: "Actual query or inspected terms",
        checkedAt: "ISO-8601 timestamp",
        outcome: "What the opened page established",
      },
    ],
    notMappableEvidence: {
      reasonCode: "One allowed notMappableContract reason",
      sourceUrls: ["https://actual-inspected-page.example/path"],
    },
  };
  ...
  if (!existsSync(templatePath)) {
    writeJson(templatePath, {
      ...
      addressCandidates: [
        ...new Set([
          ...(localRow?.locationEvidence?.addressCandidates ?? []),
          ...savedAddressEvidence.addressCandidates,
        ]),
      ],
      postalCodes: [
        ...new Set([
          ...(localRow?.locationEvidence?.postalCodes ?? []),
          ...savedAddressEvidence.postalCodes,
        ]),
      ],
      coordinateCandidates:
        localRow?.locationEvidence?.coordinateCandidates ?? [],
      evidenceInspected: [
        {
          ...
          label: officialCandidatePages[0]?.label ?? "",
          url: officialCandidatePages[0]?.url ?? "",
          query: "",
          checkedAt: "",
          outcome: "",
        },
        {
          sourceType: "host_or_authority",
          label: sourcePages[0]?.source
            ? `${sourcePages[0].source} event page`
            : "",
          url: sourcePages[0]?.url ?? "",
          query: "",
          checkedAt: "",
          outcome: "",
        },
...
function compactLocalEvidence(localRow) {
  ...
  return {
    ...
    alternatives: (localRow.alternatives ?? []).map(
      ({
        key,
        name,
        gmlIds,
  ...
  while (true) {
    ...
    if (next.action === "record-stage" && next.stage === "resolve") {
      ...
      if (
        ...
        readJson(localPath).results.some(
          (row) =>
            row.status === "candidate_matched" &&
            state.venues[row.venueId]?.stages.resolve.status === "pending",
        )
      )
        next = { action: "resolve-local" };
      else {
        const localRow = existsSync(localPath)
          ? readJson(localPath).results.find(
              (row) => row.venueId === next.venue,
            )
          : null;
        const venue = state.venues[next.venue];
        const savedRecovery =
          recoveryRecords(runDirectory(options.run)).find(
            (record) => record.venueId === next.venue,
          ) ?? null;
        if (savedRecovery) {
          recordRecoveredUnresolved(
            options.run,
        ...
        if (
          applyDeterministicEventClassification(
            options.run,
            next.venue,
            venue,
            localRow,
          ...
          executed.push({
            action: "deterministic-event-classification",
            venue: next.venue,
            exitCode: 0,
          });
          continue;
        ...
        if (
          await applyDeterministicAddressRecovery(
            options.run,
            next.venue,
            venue,
            localRow,
          ...
          executed.push({
            action: "deterministic-address-recovery",
            venue: next.venue,
            exitCode: 0,
          });
          continue;
        }
        const recoveryTemplatePath = join(
          runDirectory(options.run),
          "normalized",
          `recovery-input-${next.venue}.json`,
        );
        let recoveryTemplateReady = false;
        if (existsSync(recoveryTemplatePath)) {
          try {
            validateVenueRecoveryEvidence(
              readJson(recoveryTemplatePath),
              venue.venue,
            );
            recoveryTemplateReady = true;
          } catch {
            /* An untouched or partially filled template still requires the scoped agent intervention below. */
          }
        }
        if (recoveryTemplateReady) {
          await recordVenueRecovery({
            run: options.run,
            venue: next.venue,
            evidence: recoveryTemplatePath,
            quiet: true,
          });
          executed.push({
            action: "submit-saved-recovery-template",
            venue: next.venue,
            exitCode: 0,
          });
          continue;
        }
        const recoveryContext = venueRecoveryContext(
          options.run,
          next.venue,
          venue,
          localRow,
          ...
          `${JSON.stringify(
            {
              ...
              intervention: {
                type: "ambiguous_venue",
                venue: next.venue,
                inputVenue: venue.venue,
                inputEventIds: venue.eventIds,
                cacheKey: normalizeText(venue.venue),
                evidenceHash: branchEvidenceHash(
                  runDirectory(options.run),
                  venue,
                ),
                ...
                localEvidence: compactLocalEvidence(localRow),
                ...recoveryContext,
                recoveryCommand: `npm run event-pipeline -- record-venue-recovery --run ${options.run} --venue ${next.venue} --evidence ${recoveryContext.recoveryTemplate}`,
              },
            },
            null,
            2,
    ...
    if (!actions.has(next.action)) {
      process.stdout.write(
        `${JSON.stringify({ ...progressResponse(state), intervention: { type: "stage_not_yet_automated", ...next }, executed }, null, 2)}\n`,
      );
      process.exitCode = CONTINUE_EXIT_CODE;
      return;
    }
    ...
    if (![0, CONTINUE_EXIT_CODE].includes(child.status))
      fail(`Autonomous ${next.action} failed: ${child.stderr || child.stdout}`);
...
function verify(options) {
  ...
  const state = loadState(options.run);
  if (
    state.normalization.status !== "success" ||
    (state.deduplication && state.deduplication.status !== "success")
  )
    fail("Cannot verify before successful normalization and deduplication");
  const unfinished = Object.values(state.venues).some((venue) =>
    STAGES.some((stage) => !TERMINAL_STAGE.has(venue.stages[stage].status)),
  );
  if (unfinished) fail("Cannot verify while venue stages remain pending");
  const results = {};
  for (const [name, args] of [
    ["poiSeparation", ["run", "test:poi-separation"]],
    ["build", ["run", "build"]],
    ["eventUi", ["run", "test:event-ui"]],
  ]) {
    try {
  ...
  pipelineTrace(runDir, {
    stage: "verification",
    action: "verification_terminal",
    outcome: state.verification.status,
    entityType: "run",
    entityId: options.run,
    blocker: state.verification.error,
    nextAction: `npm run event-pipeline -- advance --run ${options.run}`,
  });
  printNext(state);
}
...
async function stageFrontend(options) {
  ...
  if (
    ...
    fail("Cannot stage frontend before every resolver branch is terminal");
  if (state.deduplication && state.deduplication.status !== "success")
    fail("Cannot stage frontend before successful post-venue deduplication");
  const plan = await prepareFrontendPlan(options.run);
  for (const outcome of plan.sourceReconciliation?.traces ?? [])
    pipelineTrace(runDir, {
      ...
      reasonCode: outcome.reasonCode ?? outcome.outcome,
      evidenceRefs: outcome.sourceRecordIds ?? [],
    });
  const preliminaryEligibility = evaluateCommitEligibility(state, {
  ...
  const boundedDiagnostic = (value) => {
    const redacted = String(redactTraceValue(String(value ?? "")))
      .replace(
        /\b(authorization|cookie|api[-_]?key|token|secret|password)\b\s*[:=]\s*[^\s,;]+/gi,
        "$1=[REDACTED]",
      )
      .trim();
    return redacted.length > 4000
      ? `[truncated ${redacted.length - 4000} chars]\n${redacted.slice(-4000)}`
      : redacted;
  };
  const execute = (name, command, args, env = process.env) => {
    ...
    pipelineTrace(runDir, {
      stage: "verification",
      action: `${name}_terminal`,
      outcome: results[name].status,
      entityType: "gate",
      entityId: name,
      counts: { exitCode: results[name].exitCode },
  ...
  if (plan.geometryChanged) {
    ...
    if (results.extraction.status === "success") {
      ...
      writeJson(
        join(runDir, "frontend/verification-assets/background-tileset.json"),
        tileset,
      );
    }
  } else
  ...
  if (results.extraction.status === "success") {
    try {
      ...
      results.combinedPoiTileset = {
        status: "success",
        command: "generated from staged approved POIs",
      };
    } catch (error) {
  ...
  const verification = {
    ...
    ...results,
    error: failed.length ? `Verification failed: ${failed.join(", ")}` : null,
  };
  ...
  if (
    ...
    const stageRefs = writeVerifiedStageHandoffs({
      runDir,
      state,
      plan,
      verification,
    ...
    state.publication = {
      decision: "publish",
      reasonCodes: [],
      candidateSnapshotId: publication.snapshotId,
      activeSnapshotId: publication.snapshotId,
      publishedAt: publication.committedAt,
    };
  } else if (failed.length) {
    for (const venue of Object.values(state.venues)) {
      if (venue.stages.resolve.status !== "success") continue;
      venue.stages.highlight = {
        status: "failed",
        outputRef: null,
    ...
    state.publication = {
      ...
      reasonCodes: [
        ...new Set([
          ...preliminaryEligibility.reasons,
          ...commitEligibility.reasons,
        ]),
      ],
    ...
    for (const venue of Object.values(state.venues)) {
      if (venue.stages.resolve.status !== "success") continue;
      for (const stage of ["highlight", "pill", "panel"])
        venue.stages[stage] = {
          status: "skipped",
          outputRef: null,
          error:
            "Incomplete source or venue snapshot was verified but not committed",
        };
    }
  }
  saveState(runDir, state);
  pipelineTrace(runDir, {
    stage: "publication",
    action: "staged_publication_terminal",
    outcome: state.publication?.decision ?? verification.status,
    entityType: "snapshot",
    entityId: state.publication?.candidateSnapshotId ?? options.run,
    reasonCode: state.publication?.reasonCodes?.join(",") || null,
    blocker: verification.error,
    nextAction: `npm run event-pipeline -- advance --run ${options.run}`,
  });
  printNext(state);
}
...
async function prepareFrontendPlan(runId) {
  ...
  const currentLandmarks = environmentRecords(
...
function finalize(options) {
  ...
  if (problems.length)
    fail(
      `Refusing to finalize an incomplete run:\n- ${problems.join("\n- ")}`,
      2,
    );
  ...
  const frontendPlan = existsSync(frontendPlanPath)
    ? readJson(frontendPlanPath)
    : null;
  pipelineTrace(runDir, {
    stage: "run",
    action: "run_finalized",
    outcome: state.overallStatus,
    entityType: "run",
    entityId: options.run,
    reasonCode: state.publication?.reasonCodes?.join(",") || null,
    blocker: state.verification?.error ?? null,
    nextAction: null,
  });
  pipelineTrace(runDir, {
    stage: "run",
    action: "admin_review_queue_reconciled",
    outcome: "success",
    entityType: "run",
    entityId: options.run,
    counts: {
      active: state.adminReviewReconciliation.activeVenueIds.length,
      superseded: state.adminReviewReconciliation.superseded,
      pending: state.adminReviewReconciliation.pending,
      deferred: state.adminReviewReconciliation.deferred,
    },
    reasonCode: null,
    blocker: null,
    nextAction: null,
...
function status(options) {
  const state = loadState(options.run);
  process.stdout.write(
    `${JSON.stringify({ ...state, ...progressResponse(state), blockers: terminalProblems(state) }, null, 2)}\n`,
  );
  if (!state.finalizedAt) process.exitCode = CONTINUE_EXIT_CODE;
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  try {
    ...
    const commands = {
      ...
      "resolve-local": resolveLocal,
      "finalize-dedup": finalizeDedupCommand,
      "reprocess-unresolved": reprocessUnresolved,
      "record-venue-recovery": recordVenueRecovery,
      "reuse-resolution-cache": reuseResolutionCacheCommand,
      "plan-frontend": planFrontend,
      "stage-frontend": stageFrontend,

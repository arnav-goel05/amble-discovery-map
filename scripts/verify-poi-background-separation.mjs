...
const sourcePath = (sourceTile) =>
  path.join(pristineRoot, sourceTile.replace(/^(tiles|optimized-tiles)\//, ""));
const backgroundPath = (sourceTile) =>
  path.join(root, sourceTile.replace(/^tiles\//, "optimized-tiles/"));
...
function batchTable(filePath) {
  ...
  return JSON.parse(
    bytes
      .subarray(start, start + batchJsonLength)
      .toString("utf8")
      .trim(),
  );
}
...
for (const poi of pois) {
  ...
  for (const entry of manifest.tiles) {
    ...
    if (sha256(background) !== entry.backgroundSha256)
      failures.push(
        `${poi.id}: background output hash changed for ${entry.sourceTile}`,
      );
    if (!(entry.poiTriangles > 0))
      failures.push(
    ...
    if (selectedIds.some((id) => !id) || !sameValues(selectedIds, entry.gmlIds))
      failures.push(
        `${poi.id}: manifest identity does not match pristine batch selection for ${entry.sourceTile}`,
      );
    if (!sameValues(poiIds, selectedIds))
      failures.push(
        `${poi.id}: POI tile identity set differs from selected pristine batches for ${entry.sourceTile}`,
      );
    if (selectedIds.some((id) => backgroundIds.includes(id)))
      failures.push(
    ...
    if (expectedBackgroundIds.length > 0 && !(entry.backgroundTriangles > 0))
      failures.push(
        `${poi.id}: unrelated identities have no background geometry for ${entry.sourceTile}`,
      );
    if (expectedBackgroundIds.length === 0 && entry.backgroundTriangles !== 0)
      failures.push(
        `${poi.id}: fully selected source tile reports unexpected background geometry for ${entry.sourceTile}`,
      );
    if (!sameValues(backgroundIds, expectedBackgroundIds))
      failures.push(
        `${poi.id}: unrelated source identities were lost or unexpected identities remain in ${background}`,
      );
    inspected += 1;
  }
}
...
if (failures.length) {
  console.error(
    `POI/background identity separation failed (${failures.length}):\n- ${failures.join("\n- ")}`,
  );
  process.exitCode = 1;
} else {
  console.log(
    `POI/background identity separation passed for ${pois.length} POIs across ${inspected} tiles.`,
  );
}

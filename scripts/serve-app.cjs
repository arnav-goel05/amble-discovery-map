...
} = require("./approved-snapshot-api-plugin.cjs");
...
const apiMiddlewares = [
  approvedSnapshotApiPlugin({ root }).middleware,
  weeklyRefreshApiPlugin().middleware,
  adminPlugin.middleware,
  voicePlugin.middleware,
  planGamePlugin.middleware,
  restaurantApiPlugin().middleware,
];
const contentTypes = {
...
function sendFile(request, response, file) {
  ...
  const isTilesetManifest =
    extension === ".json" &&
    /[/\\](?:poi-tiles|optimized-tiles|tiles)[/\\]/.test(file);
  ...
  const modifiedSince = Date.parse(
    String(request.headers["if-modified-since"] || ""),
  );
  const notModified = ifNoneMatch
    ...
    : Number.isFinite(modifiedSince) &&
      modifiedSince >= Math.floor(stat.mtimeMs / 1000) * 1000;
  if (notModified && !request.headers.range) {
    response.statusCode = 304;
    return response.end();
  }
  const range = String(request.headers.range || "").match(
    /^bytes=(\d*)-(\d*)$/,
  );
  let start = 0,
    end = stat.size - 1;
...
function serveStatic(request, response) {
  ...
  if (mountRoot) {
    ...
    if (
      mountedCandidate !== mountRoot &&
      !mountedCandidate.startsWith(`${mountRoot}${path.sep}`)
    ) {
      response.statusCode = 403;
      return response.end("Forbidden");
    }
    if (
      !fs.existsSync(mountedCandidate) ||
      !fs.statSync(mountedCandidate).isFile()
    ) {
      response.statusCode = 404;
...
server.listen(port, host, () =>
  console.log(`Amble server listening on http://${host}:${port}`),
);

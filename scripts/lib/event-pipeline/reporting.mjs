...
export const REPORT_OUTCOMES = Object.freeze([
  'mapped', 'off_map', 'carry_forward_stale', 'review', 'held', 'excluded', 'archived', 'release_rollback',
]);

export function summarizeActivityOutcomes(events = []) {
  const summary = Object.fromEntries(REPORT_OUTCOMES.map((key) => [key, 0]));
  for (const event of events) {
    if (event.lifecycleState === 'active' && event.publicPlacement === 'mapped') summary.mapped += 1;
    else if (event.lifecycleState === 'active' && event.publicPlacement === 'off_map') summary.off_map += 1;
    ...
    if (event.mappingStatus === 'pending_review' || event.reviewStatus === 'review') summary.review += 1;
  }
  return summary;
}
...
export function summarizeEvidenceLevels(events = []) {
  ...
  for (const event of events) {
    ...
    for (const contribution of event.sourceContributions ?? []) {
      if (!contribution.upgradedFrom || contribution.upgradedFrom === level) continue;
...
export function statusSummary(state, run, frontendPlan = null) {
  ...
  return {
    schemaVersion: '3.0', runId: state.runId, window: run.window, timezone: run.timezone,
    status: state.overallStatus, finalizedAt: state.finalizedAt ?? null,
    publication: state.publication ?? { decision: eligibility.eligible ? 'publish' : 'preserve_previous', reasonCodes: eligibility.reasons },
    sources: Object.fromEntries(Object.entries(state.sources ?? {}).map(([name, source]) => [name, {
      role: source.sourceRole ?? 'authoritative', mode: source.operatingMode ?? 'required', status: source.status,
      counts: source.counts ?? {}, completion: source.completion ?? null, confirmationRefs: source.confirmationRefs ?? [],
      blockerReasonCode: source.blockerReasonCode ?? null, error: source.error ?? null,
    }])),
    normalization: state.normalization, deduplication: state.deduplication ?? null,
    evidence: state.deduplication?.evidence ?? state.normalization?.evidence ?? null,
    venues: Object.fromEntries(Object.entries(state.venues ?? {}).map(([id, venue]) => [id, { venue: venue.venue, eventIds: venue.eventIds, resolve: venue.stages.resolve }])),
    verification: state.verification, reconciliation: frontendPlan?.expiry ?? null,

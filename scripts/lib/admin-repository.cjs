...
class AdminRepository {
  constructor({
    databasePath = path.join(process.cwd(), "outputs/admin/admin.sqlite"),
    clock = () => new Date(),
  } = {}) {
  ...
  createSession({ tokenHash, csrfHash, expiresAt }) {
    ...
    this.db
      .prepare(
        "INSERT INTO admin_sessions(token_hash,csrf_hash,created_at,expires_at) VALUES (?,?,?,?)",
      )
  ...
  rotateSessionCsrf(tokenHash, csrfHash) {
    this.db
      .prepare(
        "UPDATE admin_sessions SET csrf_hash=? WHERE token_hash=? AND revoked_at IS NULL",
      )
  ...
  recordLoginAttempt(subjectHash) {
    this.db
      .prepare(
        "INSERT INTO admin_login_attempts(subject_hash,attempted_at) VALUES (?,?)",
      )
      .run(subjectHash, this.now());
  }

  loginAttemptsSince(subjectHash, since) {
    return this.db
      .prepare(
        "SELECT count(*) count FROM admin_login_attempts WHERE subject_hash=? AND attempted_at>=?",
      )
      .get(subjectHash, since).count;
  }

  clearLoginAttempts(subjectHash) {
    this.db
      .prepare("DELETE FROM admin_login_attempts WHERE subject_hash=?")
      .run(subjectHash);
  }
  ...
  rowToReview(row) {
    ...
    return {
      ...
      evidenceSnapshot: parse(row.evidence_snapshot, {}),
      candidates: parse(row.candidates, []),
      status: row.status,
      decisionCandidateGmlId: row.decision_candidate_gml_id,
  ...
  upsertVenueReview(review) {
    return this.transaction(() => {
      const existing = this.db
        .prepare(
          "SELECT * FROM venue_reviews WHERE venue_id=? AND evidence_hash=?",
        )
        .get(review.venueId, review.evidenceHash);
      if (existing) return this.rowToReview(existing);
      this.db
        .prepare(
          "UPDATE venue_reviews SET status='superseded',superseded_at=? WHERE venue_id=? AND status IN ('pending','deferred')",
        )
        .run(this.now(), review.venueId);
      this.db
        .prepare(
          `INSERT INTO venue_reviews(review_id,venue_id,evidence_hash,evidence_snapshot,candidates,status,created_at)
        VALUES (?,?,?,?,?,'pending',?)`,
        )
        .run(
          review.reviewId,
          review.venueId,
          review.evidenceHash,
          json(review.evidenceSnapshot ?? {}),
          json(review.candidates ?? []),
  ...
  getVenueReview(reviewId) {
    return this.rowToReview(
      this.db
        .prepare("SELECT * FROM venue_reviews WHERE review_id=?")
        .get(reviewId),
    );
  }
  ...
  listVenueReviews({ status = "pending", cursor = null, limit = 25 } = {}) {
    ...
    const rows = this.db
      .prepare(
        `SELECT * FROM venue_reviews WHERE status=? AND (? IS NULL OR review_id>?) ORDER BY review_id LIMIT ?`,
      )
      .all(status, cursor, cursor, bounded + 1);
    return {
      records: rows.slice(0, bounded).map((row) => this.rowToReview(row)),
      nextCursor: rows.length > bounded ? rows[bounded - 1].review_id : null,
    };
  }

  reconcileVenueReviewQueue(activeReviews = []) {
    const refs = activeReviews
      .map((item) =>
        typeof item === "string" ? { venueId: item, evidenceHash: null } : item,
      )
    ...
    return this.transaction(() => {
      const active = new Set(
        refs.map(
          ({ venueId, evidenceHash }) => `${venueId}\0${evidenceHash ?? "*"}`,
        ),
      );
      const rows = this.db
        .prepare(
          "SELECT review_id,venue_id,evidence_hash FROM venue_reviews WHERE status IN ('pending','deferred')",
        )
        .all();
      const stale = rows.filter(
        (row) =>
          !active.has(`${row.venue_id}\0*`) &&
          !active.has(`${row.venue_id}\0${row.evidence_hash}`),
      );
      const supersede = this.db.prepare(
        "UPDATE venue_reviews SET status='superseded',superseded_at=? WHERE review_id=?",
      );
      for (const row of stale) supersede.run(this.now(), row.review_id);
      const superseded = stale.length;
      const pending = this.db
        .prepare(
          "SELECT COUNT(*) AS count FROM venue_reviews WHERE status='pending'",
        )
        .get().count;
      const deferred = this.db
        .prepare(
          "SELECT COUNT(*) AS count FROM venue_reviews WHERE status='deferred'",
        )
        .get().count;
  ...
  decideVenueReview(reviewId, decision) {
    return this.transaction(() => {
      ...
      const prior = this.db
        .prepare(
          "SELECT operation,response FROM admin_idempotency WHERE idempotency_key=?",
        )
        .get(decision.idempotencyKey);
      ...
      if (!review)
        throw new AdminRepositoryError(
          "venue_review_not_found",
          "Venue review was not found",
          404,
        );
      if (review.evidenceHash !== decision.evidenceHash)
        throw new AdminRepositoryError(
          "venue_review_stale",
          "Venue evidence changed; refresh before deciding",
          409,
        );
      if (!["pending", "deferred"].includes(review.status))
        throw new AdminRepositoryError(
          "venue_review_already_decided",
          "Venue review was already decided",
          409,
        );
      const status = {
        approve: "approved",
      ...
      if (
        ...
        !review.candidates.some(
          (candidate) =>
            candidate.gmlId === decision.candidateGmlId ||
            candidate.gmlIds?.includes(decision.candidateGmlId),
        )
      ) {
        throw new AdminRepositoryError(
          "venue_review_candidate_invalid",
          "Selected candidate is absent from current evidence",
          409,
        );
      }
      if (
        ...
        throw new AdminRepositoryError(
          "venue_review_reason_required",
          "A decision reason is required",
        );
      const decidedAt = this.now();
      this.db
        .prepare(
          `UPDATE venue_reviews SET status=?,decision_candidate_gml_id=?,decision_reason=?,idempotency_key=?,decided_at=? WHERE review_id=?`,
        )
        .run(
          status,
          status === "approved" ? decision.candidateGmlId : null,
      ...
      const response = {
        review: this.getVenueReview(reviewId),
        pipelineReconciliationRequired: true,
        idempotent: false,
      };
      this.db
        .prepare(
          "INSERT INTO admin_idempotency(idempotency_key,operation,response,created_at) VALUES (?,?,?,?)",
        )
  ...
  approvedVenueReviews() {
    return this.db
      .prepare(
        "SELECT * FROM venue_reviews WHERE status='approved' ORDER BY decided_at",
      )
      .all()
  ...
  performIdempotent(idempotencyKey, operation, work) {
    return this.transaction(() => {
      ...
      this.db
        ...
        .run(idempotencyKey, operation, json(response), this.now());
      return response;
    });
  }

...
export function canonicalRenderedUrl(value) {
  ...
  if (url.protocol !== "https:" || url.username || url.password)
    throw Object.assign(
      new Error("Rendered target must be credential-free HTTPS"),
...
export async function assertPublicUrl(value, { resolver = lookup } = {}) {
  ...
  if (isPrivateAddress(url.hostname))
    throw Object.assign(new Error("Private destination rejected"), {
      code: "unsafe_destination",
    });
  ...
  if (
    ...
    throw Object.assign(
      new Error("Destination did not resolve exclusively to public addresses"),
      { code: "unsafe_destination" },
    );
  }
  return canonicalUrl;
...
const RETRYABLE_RESULT_ERRORS = new Set([
  "bot_blocked",
  "proxy_error",
  "target_unreachable",
  "timeout",
...
async function readBoundedResponseText(response, maximumResponseBytes) {
  ...
  if (
    ...
    throw Object.assign(new Error("TinyFish response exceeded size limit"), {
...
export function createTinyfishFetchClient({
  ...
  if (
    ...
    batchSize < 1 ||
    batchSize > 10 ||
    maximumUrlsPerMinute < 1 ||
    maximumUrlsPerMinute >= 150
  )
    throw new Error("Invalid TinyFish free-tier bounds");
  if (!["html", "json", "markdown"].includes(format))
    throw new Error("Invalid TinyFish response format");
  if (ttl !== undefined && (!Number.isInteger(ttl) || ttl < 0))
    throw new Error("Invalid TinyFish cache freshness");
  for (const selectors of [includeSelectors, excludeSelectors]) {
    if (
      ...
      throw new Error("Invalid TinyFish selector scope");
  }
  let lastBatchAt = 0;
  ...
  async function fetchBatch(values, context = {}) {
    ...
    const requestIncludeSelectors =
      requestOptions.includeSelectors ?? includeSelectors;
    const requestExcludeSelectors =
      requestOptions.excludeSelectors ?? excludeSelectors;
    if (!["html", "json", "markdown"].includes(requestFormat))
      throw new Error("Invalid TinyFish response format");
    if (
      requestTtl !== undefined &&
      (!Number.isInteger(requestTtl) || requestTtl < 0)
    )
      throw new Error("Invalid TinyFish cache freshness");
    for (const selectors of [
      requestIncludeSelectors,
      requestExcludeSelectors,
    ]) {
      if (
        !Array.isArray(selectors) ||
        selectors.length > 20 ||
    ...
    const urls = [
      ...new Set(
        await Promise.all(
          values.map((value) => assertPublicUrl(value, { resolver })),
        ),
      ),
    ].sort();
    if (urls.length > batchSize)
      throw Object.assign(new Error(`TinyFish batch exceeds ${batchSize}`), {
        code: "batch_limit_exceeded",
      });
    const delay = Math.max(
      0,
      lastBatchAt + Math.ceil(delayPerUrl * urls.length) - now(),
    );
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      logger({
        ...
        ttl: requestTtl ?? null,
        includeSelectors: requestIncludeSelectors.length,
        excludeSelectors: requestExcludeSelectors.length,
        stage: context.stage,
      ...
      try {
        const response = await fetchImpl(endpoint, {
          ...
          body: JSON.stringify({
            ...
            ...(requestIncludeSelectors.length
              ? { include_selectors: requestIncludeSelectors }
              : {}),
            ...(requestExcludeSelectors.length
              ? { exclude_selectors: requestExcludeSelectors }
              : {}),
        ...
        if (!response.ok) {
          ...
          if (
            ...
            errors.length &&
            errors.every((item) =>
              RETRYABLE_RESULT_ERRORS.has(item.error ?? item.code),
            ) &&
            attempt < maxAttempts
          ) {
            lastError = Object.assign(
              new Error("TinyFish returned only transient per-URL failures"),
            ...
            logger({
              ...
              attempt,
              urls: urls.length,
              reason: "transient_per_url_errors",
        ...
        lastError =
          error.name === "AbortError"
            ? Object.assign(new Error("TinyFish request timed out"), {
                code: "source_unavailable",
              })
            : error;
        if (
          attempt === maxAttempts ||
          !["source_unavailable", "persistent_rate_limit"].includes(
            lastError.code,
          )
        )
  ...
  async function fetchUrls(values, context = {}) {
    ...
    const output = [];
    for (let index = 0; index < canonical.length; index += batchSize)
      output.push(
        await fetchBatch(canonical.slice(index, index + batchSize), context),
      );
    return output;
  }

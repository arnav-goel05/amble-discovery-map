...
async function boundedText(response, maximumResponseBytes) {
  ...
  if (Number.isFinite(declared) && declared > maximumResponseBytes) {
    ...
    throw Object.assign(new Error("Direct HTML response exceeded size limit"), {
  ...
  if (!reader) {
    ...
    if (Buffer.byteLength(text) > maximumResponseBytes)
      throw Object.assign(
        new Error("Direct HTML response exceeded size limit"),
        { code: "response_too_large" },
      );
    return text;
  }
...
async function assertDirectPublicUrl(value, resolver) {
  ...
  if (url.protocol !== "https:" || url.username || url.password)
    throw Object.assign(
      new Error("Direct target must be credential-free HTTPS"),
      {
...
function robotsAllows(text, path, userAgent) {
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    if (user) {
      applies =
        user === "*" || userAgent.toLowerCase().includes(user.toLowerCase());
      continue;
    }
...
export function createDirectHtmlFetchClient({
  ...
  async function request(url, { signal } = {}) {
    ...
    if (!allowedHost(canonical, officialDomains))
      throw Object.assign(
        new Error("Direct target is outside official source domains"),
        { code: "official_domain_rejected" },
      );
  ...
  async function checkRobots(url, signal) {
    ...
    if (!robotsCache.has(origin)) {
      ...
      robotsCache.set(
        ...
        response.ok
          ? await boundedText(
              response,
              Math.min(maximumResponseBytes, 512 * 1024),
            )
          : "",
      );
    }
    if (!robotsAllows(robotsCache.get(origin), parsed.pathname, userAgent))
      throw Object.assign(
        new Error("Direct retrieval disallowed by robots.txt"),
  ...
  async function fetchOne(input, context) {
    ...
    if (pageCache.has(requestedUrl)) {
      logger({
        action: "direct_html_cache_reused",
        sourceName: context.sourceName,
        stage: context.stage,
        entityId: context.entityId,
        urlHash: sha(requestedUrl),
      });
      ...
      for (let redirect = 0; redirect <= maximumRedirects; redirect += 1) {
        ...
        if ([301, 302, 303, 307, 308].includes(response.status)) {
          if (redirect === maximumRedirects)
            throw Object.assign(
              new Error("Direct HTML redirect limit exceeded"),
              { code: "redirect_limit_exceeded" },
            );
          const location = response.headers.get("location");
          if (!location)
            throw Object.assign(
              new Error("Direct HTML redirect omitted location"),
        ...
        if (!response.ok)
          throw Object.assign(
            new Error(`Direct HTML returned HTTP ${response.status}`),
            {
              code:
        ...
        if (
          ...
          throw Object.assign(new Error("Direct response was not HTML"), {
  ...
  async function fetchBatch(urls, context = {}) {
      ...
      try {
        ...
        const normalized =
          error.name === "AbortError"
            ? Object.assign(new Error("Direct HTML request timed out"), {
                code: "timeout",
              })
            : error;
        errors.push({
          url,
          code: normalized.code ?? "source_unavailable",
          status: normalized.status ?? null,
          message: normalized.message,
        });
        logger({
          action: "direct_html_failed",
          sourceName: context.sourceName,
          stage: context.stage,
          entityId: context.entityId,
          reasonCode: normalized.code ?? "source_unavailable",
    ...
    return {
      ...
      payloadHash: sha(
        JSON.stringify(
          results.map(({ url, final_url, contentHash }) => ({
            url,
            final_url,
            contentHash,
          })),

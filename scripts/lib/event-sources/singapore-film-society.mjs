...
export const singaporeFilmSocietyAdapter = {
  ...
  detailLinks(result, source, url) {
    return canonicalLinks(result, { baseUrl: url, pathPattern: "^/schedule/" });
  },
  detail(result, source, detailUrl) {
    return parseAuthorityDetail(result, {
      source,
      detailUrl,
      classify: ({ dateText, venue, document }) => {
        const text = normalized(document.text);
        if (/sign in to view|login required/.test(text) && !dateText)

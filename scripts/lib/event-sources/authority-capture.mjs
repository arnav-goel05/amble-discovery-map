...
export function createAuthorityCaptureIndex({
  runDir,
  runId,
  window,
  ...
  const value = existsSync(path)
    ...
    : { schemaVersion: "1.0", runId, window, entries: [] };
  if (
    value.runId !== runId ||
    JSON.stringify(value.window) !== JSON.stringify(window)
  )
    throw new Error("Authority index belongs to another run/window");
  const save = () => atomicJson(path, value);
  if (!existsSync(path)) save();
  const find = (url) =>
    value.entries.find(
      (entry) => entry.canonicalUrl === canonicalRenderedUrl(url),
    );
  return {
    path,
    reserve(requestedUrl) {
      ...
      if (!entry) {
        ...
        save();
      } else if (!entry.requestedAliases.includes(requestedUrl)) {
        entry.requestedAliases.push(requestedUrl);
        entry.requestedAliases.sort();
        save();
      }
    ...
    complete(
      ...
      entry.redirectChain = redirectChain;
      entry.captureRef = `raw/authority/${sha(canonicalUrl)}.response.json`;
      atomicJson(join(runDir, entry.captureRef), payload);
      save();
      return structuredClone(entry);
    },
    parsed(url, parserHash, fixture) {
      ...
      if (!entry || entry.status !== "validated")
        throw new Error("Validated authority capture required");
      const fixtureRef = `raw/authority/${sha(entry.canonicalUrl)}.${parserHash.slice(0, 12)}.json`;
      atomicJson(join(runDir, fixtureRef), fixture);
      entry.parsed[parserHash] = fixtureRef;
      save();
      return fixtureRef;
    },
    reusable(url, parserHash = null) {
      ...
      if (
        !entry ||
        entry.status !== "validated" ||
        entry.retrievalDefinitionHash !== retrievalDefinitionHash
      )
        return null;

...
export const rootsHanAdapter = {
  ...
  listing(result, source, url = source.listing.url) {
    return {
      ...
      ...terminalPagination(result, { baseUrl: url, nextLabels: ["next"] }),
    };
  },
  detail(result, source, detailUrl) {
    return parseAuthorityDetail(result, {
      source,
      detailUrl,
      performanceBuilder: rootsPerformances,
      classify: ({ timeText, mode, dateText, document }) => {
        const text = normalized(document.text);
        if (/open call|volunteer|get involved|resource/.test(text))
          return "non_event_editorial";
        if (mode === "online") return "not_physical_sg";
        if (/various timings/i.test(timeText ?? "") || !dateText)
          return "undated_review";
        return null;
      },
    });
  },

...
export function assertPaidExceptionAllowed(policy, providerId, { featureId, url } = {}) {
  if (policy?.schemaVersion !== "1.0" || !Array.isArray(policy.providers)) fail("provider_policy_invalid", "Provider policy has an unsupported shape");
  const provider = policy.providers.find(({ id }) => id === providerId);
  if (!provider) fail("provider_unapproved", `Provider ${providerId} is not approved`);
  if (providerId !== "openai-realtime" || provider.costClass !== "paid-exception" || provider.featureId !== "004-conversational-voice-map" || provider.policyFile !== "data/realtime-voice-policy.json") {
    fail("provider_paid_exception_invalid", `Provider ${providerId} is not the reviewed paid exception`);
  }
  if (provider.enabled !== true || provider.owner !== "OpenAI" || !Array.isArray(provider.domains) || provider.domains.length !== 1 || provider.domains[0] !== "api.openai.com") {
    fail("provider_paid_exception_invalid", `Provider ${providerId} has invalid exception evidence`);
  }
  if (featureId !== provider.featureId) fail("provider_paid_exception_scope_invalid", `Provider ${providerId} is not approved for feature ${featureId}`);
  if (url) {
    let hostname;
    try { hostname = new URL(url).hostname.toLowerCase(); }
    catch { fail("provider_url_invalid", `Provider ${providerId} URL is invalid`); }
    if (hostname !== "api.openai.com") fail("provider_domain_unapproved", `Domain ${hostname} is not approved for ${providerId}`);
  }
  return Object.freeze({ ...provider, domains: Object.freeze([...provider.domains]) });
}
...
export function loadEventAuthorityRegistry(file) {
  ...
  catch (error) { fail("authority_registry_unreadable", `Event authority registry could not be loaded: ${error.message}`); }
  if (registry?.schemaVersion !== "1.0" || !Array.isArray(registry.entries)) fail("authority_registry_invalid", "Event authority registry has an unsupported shape");
  const ids = new Set();
  for (const entry of registry.entries) {
    if (!entry?.authorityId || ids.has(entry.authorityId) || entry.status !== "approved" || !entry.owner
      || !["organizer", "venue", "institution", "government", "authorized_ticketing"].includes(entry.authorityType)
      || !Array.isArray(entry.domains) || entry.domains.length === 0 || !Array.isArray(entry.detailPathPatterns)) {
      fail("authority_registry_invalid", "Authority entries require unique identity, approved status, owner, type, domains, and detail paths");
    }
    for (const pattern of entry.detailPathPatterns) {
      try { new RegExp(pattern); } catch { fail("authority_registry_invalid", `Invalid detail path pattern for ${entry.authorityId}`); }
    }
...
export function assertAuthorityUrlAllowed(registry, value) {
  let url;
  try { url = new URL(value); } catch { fail("authority_url_invalid", "Authority URL is invalid"); }
  if (url.protocol !== "https:" || url.username || url.password || isPrivateAddress(url.hostname)) fail("authority_url_unsafe", "Authority URL must be public HTTPS without credentials");
  const hostname = url.hostname.toLowerCase();
  const entry = registry?.entries?.find((candidate) => candidate.domains.some((domain) => hostname === domain || hostname.endsWith(`.${domain}`))
    && candidate.detailPathPatterns.some((pattern) => new RegExp(pattern).test(url.pathname)));
  if (!entry) fail("authority_domain_review", `Authority URL ${hostname}${url.pathname} is not in the reviewed registry`);
  return { url: url.href, authorityId: entry.authorityId, owner: entry.owner, authorityType: entry.authorityType };
}

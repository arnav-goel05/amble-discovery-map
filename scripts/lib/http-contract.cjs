...
function successEnvelope(
  data,
  {
    ...
    stale = false,
    warning = null,
    source = { id: "whats-here", costClass: "free" },
  } = {},
) {
...
function readJsonBody(
  request,
  { maxBytes = DEFAULT_MAX_BODY_BYTES, required = true } = {},
) {
...
function publicError(error) {
  if (error instanceof HttpContractError)
    return {
      status: error.status,
      body: errorEnvelope(error.code, error.message),
    };
  const code =
  ...
  const known = new Map([
    [
      ...
      [503, "Approved event data is currently unavailable."],
    ],
    [
  ...
  const [status, message] = known.get(code) ?? [
    500,

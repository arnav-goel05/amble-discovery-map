...
export function extractAddressEvidence(values) {
  const text = (Array.isArray(values) ? values : [values]).map(plainText).filter(Boolean).join('\n');
  ...
  const addressCandidates = candidates.filter((candidate) => !candidates.some((other) => other !== candidate
    && candidate.toLowerCase().includes(other.toLowerCase()) && other.length + 8 < candidate.length));
  const units = [...new Set([...text.matchAll(/#\s*\d{1,3}\s*-\s*[a-z0-9]{1,5}/gi)].map((match) => match[0].replace(/\s+/g, '')))];
  return { postalCodes, addressCandidates, units };
}

export function preferAuthoritativeRecovery(sourceValues = [], deterministicValues = [], reviewedValues = []) {

...
export function renderPlanPreview({
  ...
  const minutes = distance
    ? Math.max(1, Math.round((distance / speeds[travelMode]) * 60))
    : 0;
...
export function renderPlanRoutes({
  ...
  if (!routeUrls.length) {
    const button = element(
      "button",
      "plan-builder__maps-link plan-builder__maps-link--disabled",
    );
    button.type = "button";
  ...
  routeUrls.forEach((href, index, links) => {
    ...
    link.append(
      ...
      element(
        ...
        "",
        links.length > 1
          ? `Open Google Maps route ${index + 1}/${links.length}`
    ...
    link.dataset.capabilityId = "plan.openroute";
    link.dataset.segmentIndex = String(index);
    if (typeof onOpenRoute === "function")
      link.addEventListener("click", (event) => onOpenRoute(event, index));

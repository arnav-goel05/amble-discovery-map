...
import { addPlanStop, createPlanState, movePlanStop, planStopKey, removePlanStop } from "./planning/plan-model.js";
import { element, iconButton, renderPlanPreview, renderPlanRoutes } from "./planning/plan-view.js";
...
export function addPlanBuilder({
  ...
  if (document.getElementById("plan-builder-button")) return { finalize() {} };
  ...
  const modeLabelText = element("span", "plan-builder__field-label", "Travel mode");
  const mode = element("select", "plan-builder__mode");
  for (const value of ["walking", "driving", "bicycling", "transit"]) { const option = element("option", "", value[0].toUpperCase() + value.slice(1)); option.value = value; mode.appendChild(option); }
  modeLabel.append(modeLabelText, mode);
  const status = element("p", "plan-builder__status", "Add an event or restaurant to begin."); status.setAttribute("role", "status"); status.setAttribute("aria-live", "polite");
  ...
  const renderPreview = () => {
    renderPlanPreview({ container: preview, stops, currentLocation, travelMode: mode.value });
  };
  ...
  function moveDrag(event) {
    ...
    for (const [index, item] of [...list.querySelectorAll(".plan-builder__stop:not(.plan-builder__stop--origin)")].entries()) {
      item.classList.toggle("is-drop-target", index === nearestIndex && item !== dragState.item);
    }
  }
  ...
  const beginDrag = (event, item, index, stop) => {
    ...
    event.preventDefault();
    dragState = {
      centers: [...list.querySelectorAll(".plan-builder__stop:not(.plan-builder__stop--origin)")].map((node) => {
    ...
    catch { /* Synthetic tests and legacy engines may not expose an active pointer. */ }
  ...
  const setOpen = (open) => {
    ...
    if (open) close.focus();
    if (open && locationState === "idle") requestCurrentLocation();
    if (!open && wasOpen) announceOverlayClosed("plan-builder");
  };

  const renderRoutes = () => {
    renderPlanRoutes({ container: maps, stops, currentLocation, travelMode: mode.value });
  };
  ...
  const renderCurrentLocation = () => {
    ...
    const copyNode = element("button", "plan-builder__stop-focus plan-builder__stop-copy");
    copyNode.type = "button";
    copyNode.ariaLabel = currentLocation ? "Show my location on map" : "Use my current location";
    const locationCopy = locationState === "ready"
  ...
  const render = () => {
    ...
    stops.forEach((stop, index) => {
      const item = element("li", "plan-builder__stop"); item.dataset.planStop = planStopKey(stop);
      const copyNode = element("button", "plan-builder__stop-focus plan-builder__stop-copy");
      copyNode.type = "button";
      copyNode.ariaLabel = `Show ${stop.title} on map`;
      copyNode.append(element("strong", "plan-builder__stop-title", stop.title), element("span", "plan-builder__stop-place", `${stop.type === "event" ? "Event" : "Food"} · ${stop.place}`));
      const controls = element("div", "plan-builder__stop-controls");
      const drag = iconButton("plan-builder__drag-handle", `Drag ${stop.title} to reorder`, "dots-six-vertical");
      const remove = iconButton("plan-builder__stop-button plan-builder__stop-button--remove", `Remove ${stop.title}`, "x");
      remove.onclick = () => { planState = removePlanStop(planState, planStopKey(stop)); stops = planState.stops; render(); };
      drag.addEventListener("pointerdown", (event) => beginDrag(event, item, index, stop));
      item.onclick = (event) => {
        if (event.target.closest(".plan-builder__stop-controls")) return;
        showStopOnMap(stop);
      };
      controls.append(drag, remove); item.append(copyNode, controls); list.appendChild(item);
    });
    status.textContent = stops.length ? `${stops.length} stop${stops.length === 1 ? "" : "s"}. Drag stops to reorder.` : "Add an event or restaurant to begin.";
    renderRoutes();
    renderPreview();
  };
  ...
  const add = (event) => {
    ...
    if (added.reason === "duplicate") { setOpen(true); status.textContent = `${stop.title} is already in your plan.`; return; }
    if (added.reason === "limit") { setOpen(true); status.textContent = "A plan can contain at most 20 stops."; return; }
    planState = added.state; stops = planState.stops; render(); setOpen(true); status.textContent = `${stop.title} added. Your Google Maps route is ready below.`;
  };

  button.onclick = () => setOpen(panel.hidden); close.onclick = () => setOpen(false);
  const stopWatchingOverlays = closeWhenAnotherOverlayOpens("plan-builder", () => setOpen(false));
  const onKey = (event) => { if (event.key === "Escape" && !panel.hidden) setOpen(false); };
  document.addEventListener("keydown", onKey); window.addEventListener("whats-here:add-to-plan", add);
  for (const type of ["pointerdown", "mousedown", "touchstart", "wheel", "dblclick"]) panel.addEventListener(type, (event) => event.stopPropagation());
  mode.addEventListener("change", () => { renderRoutes(); renderPreview(); status.textContent = `${mode.value[0].toUpperCase() + mode.value.slice(1)} route ready in Google Maps.`; });
  render(); document.body.dataset.planBuilder = "mounted";
  return { id: "plan-builder", finalize() { finishDrag(false); stopWatchingOverlays(); document.removeEventListener("keydown", onKey); window.removeEventListener("whats-here:add-to-plan", add); panel.remove(); button.remove(); } };
}

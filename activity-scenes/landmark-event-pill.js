...
const searchableText = (...values) => values.flat().filter(Boolean).join(" ").toLocaleLowerCase();
...
function distanceFromMapCenter(map, landmark) {
  ...
  if (!Number.isFinite(point?.x) || !Number.isFinite(point?.y)) return Number.POSITIVE_INFINITY;
  return Math.hypot(point.x - window.innerWidth / 2, point.y - window.innerHeight / 2);
}

const CATEGORY_RULES = [
  ["Workshops & Classes", /\b(workshops?|classes?|courses?|lessons?|masterclasses?|seminars?|talks?|lectures?|conferences?|beginners?|painting experience|art education)\b/i],
  ["Exhibitions", /\b(exhibitions?|galleries|gallery|museums?|installations?|visual arts?|artscience|showcases?)\b/i],
  ["Performances", /\b(concerts?|music|musical|orchestras?|symphon(?:y|ies)|recitals?|singers?|bands?|jazz|opera|theatre|theater|plays?|drama|stage|dance|ballet|choreograph(?:y|ed)?|comedy|comedian|stand[ -]?up|storytelling|sing[ -]?along)\b/i],
  ["Tours & Experiences", /\b(tours?|trails?|walks?|rides?|adventures?|hunts?|admissions?|observation deck|attractions?|experiences?|excursions?)\b/i],
];

export function eventCategory(event = {}) {
  const list = (value) => Array.isArray(value) ? value : value ? [value] : [];
  const explicit = [event.category, event.genre, event.eventType, event.type, ...list(event.categories), ...list(event.tags)]
    .flat()
    .filter((value) => typeof value === "string" && value.trim());
  const primaryText = [...explicit, event.title, event.venue].filter(Boolean).join(" ");
  const primaryMatch = CATEGORY_RULES.find(([, rule]) => rule.test(primaryText));
  if (primaryMatch) return primaryMatch[0];
  const descriptionMatch = CATEGORY_RULES.find(([, rule]) => rule.test(event.description || ""));
...
function updatePosition(map, state, minZoom, onHidden) {
  const point = map.project([state.landmark.anchor.lng, state.landmark.anchor.lat]);
  ...
  const isNavigationTarget = state.root.classList.contains("is-navigation-target");
  const isNearLocation = state.matchesSearch &&
  ...
  if (state.isVisible && !isNearLocation) {
    const wasInteracting = state.card.contains(document.activeElement) || state.card.getAttribute("aria-expanded") === "true";
    if (wasInteracting) {
      onHidden?.({ landmark: state.landmark, trigger: state.card });
      const visibleMapControl = document.querySelector(".maplibregl-ctrl-zoom-in");
  ...
  state.root.style.setProperty("--pill-card-offset-x", `${Math.round(cardCenter - point.x)}px`);
...
export function createLandmarkEventPillLayer({
  ...
  const recordPositionPass = () => {
    ...
    body.dataset.landmarkEventPillPositionUpdateCount = String(
      Number(body.dataset.landmarkEventPillPositionUpdateCount || 0) + states.length,
    );
  };
  ...
  const rotateEvents = () => {
    ...
    for (const state of states) {
      if (state.visibleEventIndices.length < 2 || now - state.lastRotationAt < rotationMs) continue;
      const currentVisibleIndex = state.visibleEventIndices.indexOf(state.activeIndex);
      state.activeIndex = state.visibleEventIndices[(currentVisibleIndex + 1) % state.visibleEventIndices.length] ?? 0;
  ...
  function scheduleRotation() {
    ...
    const rotatingStates = states.filter((state) => state.visibleEventIndices.length > 1);
    if (!rotatingStates.length) {
      rotationTimer = null;
      return;
    }
    const now = performance.now();
    const nextDue = Math.min(...rotatingStates.map((state) => state.lastRotationAt + rotationMs));
    rotationTimer = setTimeout(rotateEvents, Math.max(0, nextDue - now));
  }

  const mapEvents = ["move", "zoom", "resize"];
  for (const eventName of mapEvents) map.on?.(eventName, schedulePositionUpdate);
  window.addEventListener("resize", schedulePositionUpdate);

  const normalizedInput = (landmark, sourceEvents) => (Array.isArray(sourceEvents) ? sourceEvents : [])
    .map((sourceEvent, index) => ({ normalized: normalizeEvent(sourceEvent, landmark.id, index), sourceEvent }))
    .filter(({ normalized }) => normalized);

  const findState = (landmarkId) => states.find((state) => state.landmark.id === landmarkId);
  ...
  const remove = (landmarkId) => {
    ...
    onEventsChanged?.({ landmark: state.landmark, sourceEvents: [], trigger: state.card });
    state.root.remove();
    scheduleRotation();
    return true;
  };
  ...
  const add = ({ landmark, sourceEvents }) => {
    if (!landmark?.id || !landmark?.label || !landmark?.anchor || document.getElementById(`${landmark.id}-event-pill`)) {
      return null;
    }
    ...
    root.classList.toggle("is-navigation-target", landmark.id === navigationTargetId);
  ...
  const upsert = ({ landmark, sourceEvents }) => {
    ...
    const selectedIndex = existing.events.findIndex((event) => event.id === selectedId);
    ...
    existing.root.setAttribute("aria-label", `${landmark.label} upcoming events`);
    existing.card.setAttribute("aria-label", `Open ${landmark.label} event details`);
    renderState(existing);
    updatePosition(map, existing, minZoom, onHidden);
    scheduleRotation();
    onEventsChanged?.({ landmark, sourceEvents: existing.sourceEvents, trigger: existing.card });
  ...
  const reconcile = ({ landmarks, runStatus }) => {
    ...
    for (const state of [...states]) if (!incomingIds.has(state.landmark.id)) remove(state.landmark.id);
    return true;
  };

  const applyDiscoveryResult = (discoveryResult) => {
    const identities = discoveryResult?.identities instanceof Set ? discoveryResult.identities : new Set();
    let matchedEvents = 0;
    let matchedLandmarks = 0;
    const matchesByState = new Map();
    for (const state of states) {
      const matches = state.events.map((event, index) => ({ index, identity: eventIdentity(state.landmark.id, event.id) }))
      ...
      if (state.matchesSearch && !matches.some((entry) => entry.index === state.activeIndex)) {
        if (state.card.getAttribute("aria-expanded") === "true") onHidden?.({ landmark: state.landmark, trigger: state.card });
    ...
    matchedLandmarks = new Set(results.map(({ landmarkId }) => landmarkId)).size;
  ...
  const setFilters = ({ query: value = "", categories = [] } = {}) => {
    const model = createEventDiscoveryModel(states.map((state) => ({ ...state.landmark, events: state.sourceEvents })), { categoryOf: eventCategory });
  ...
  const categories = () => [...new Set(states.flatMap((state) => state.events.map((event) => event.category)))].sort();

  const destroy = () => {
    if (positionFrame !== null) cancelAnimationFrame(positionFrame);
    if (positionFallback !== null) clearTimeout(positionFallback);
    if (rotationTimer !== null) clearTimeout(rotationTimer);
    for (const eventName of mapEvents) map.off?.(eventName, schedulePositionUpdate);
  ...
  return { add, applyDiscoveryResult, categories, destroy, reconcile, remove, selectResult, setFilters, setNavigationTarget, setSearchQuery, upsert };
}

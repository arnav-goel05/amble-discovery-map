...
test("the fixed 45 degree camera stays aligned while the map zooms", async ({
  ...
  await expect
    .poll(
      () => page.evaluate(() => document.body.dataset.backgroundViewLoaded),
      { timeout: 20_000 },
    )
    .toBe("true");
  await expect
    .poll(
      () =>
        page.evaluate(() => {
          const layer = (window._map.__deck?.props?.layers || [])
            .flat(Infinity)
            .filter(Boolean)
  ...
  await page.evaluate(() => {
    ...
    const sample = () => {
      ...
      const deckPoint = map.__deck.viewManager
        .getViewports()[0]
        .project([...anchor, 0]);
      window.__mapRenderSyncSamples.push({
        ...
        projectionDelta: Math.hypot(
          mapPoint.x - deckPoint[0],
          mapPoint.y - deckPoint[1],
        ),
  ...
  await expect
    .poll(() => page.evaluate(() => Math.abs(window._map.getZoom() - 15.3)))
    .toBeLessThan(0.01);
  await expect.poll(() => page.evaluate(() => window._map.getPitch())).toBe(45);
  await expect
    .poll(
      () => page.evaluate(() => document.body.dataset.tileRefinementState),
      { timeout: 20_000 },
    )
    .toBe("full-detail");
  await expect
    .poll(() => page.evaluate(() => window.__mapRenderSyncSamples.length))
    .toBeGreaterThan(5);
  ...
  const result = await page.evaluate(() => {
    ...
    return {
      ...
      maximumDelta: Math.max(
        ...samples.flatMap(({ map, deck }) =>
          map.map((value, index) => Math.abs(value - deck[index])),
        ),
      ),
      maximumProjectionDelta: Math.max(
        ...samples.map(({ projectionDelta }) => projectionDelta),
      ),
      minimumPitch: Math.min(...samples.map(({ map }) => map[3])),
      maximumPitch: Math.max(...samples.map(({ map }) => map[3])),
      refinementStates: [
        ...new Set(samples.map(({ refinement }) => refinement)),
      ],
      traversalStates: [...new Set(samples.map(({ traversal }) => traversal))],
      backgroundLoadTiles: [
        ...new Set(
          samples.map(({ backgroundLoadTiles }) => backgroundLoadTiles),
        ),
      ],
      ...
      backgroundScreenSpaceErrors: [
        ...new Set(
          samples.map(
            ({ backgroundScreenSpaceError }) => backgroundScreenSpaceError,
          ),
        ),
      ],
      poiScreenSpaceErrors: [
        ...new Set(
          samples.map(({ poiScreenSpaceError }) => poiScreenSpaceError),
        ),
      ],
    };
  });

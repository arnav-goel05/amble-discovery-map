...
test("empty snapshots keep the mobile toolbar compact and hide laptop-only map controls", async ({
  ...
  const mobileToolbar = await page.evaluate(() => {
    ...
    const buttons = [
      ...document.querySelectorAll(
        ".landmark-event-search__category, .landmark-event-search__actions > button",
      ),
    ].map((button) => button.getBoundingClientRect());
    const dateFilter = document
      .querySelector(".landmark-event-search__filters")
      .getBoundingClientRect();
    return {
  ...
  await expect(page.locator(".landmark-event-search__filters")).toHaveCSS(
    "border-left-width",
    "0px",
  );
  await expect(page.locator(".landmark-event-search__filters")).toHaveCSS(
...
test("only the secret-location filter remains and location types move into activity details", async ({
  ...
  await search.focus();
  await expect(
    page.getByRole("button", { name: "Mystery Location" }),
  ).toBeVisible();

...
const verificationOrigin = process.env.EVENT_PIPELINE_BROWSER_ORIGIN ?? 'http://127.0.0.1:4174';
const viteFsUrl = (value) => new URL(`/@fs${path.resolve(value).split(path.sep).join('/')}`, verificationOrigin).href;
const stagedPois = pois.map((poi) => ({ ...poi, data: fs.existsSync(path.join(runDir, 'frontend/assets/public', poi.data)) ? viteFsUrl(path.join(runDir, 'frontend/assets/public', poi.data)) : poi.data }));
const stagedBackground = runDir && fs.existsSync(path.join(runDir, 'frontend/verification-assets/background-tileset.json'))
  ? viteFsUrl(path.join(runDir, 'frontend/verification-assets/background-tileset.json')) : new URL('/optimized-tiles/tileset.json', verificationOrigin).href;
const stagedCombinedPois = runDir && fs.existsSync(path.join(runDir, 'frontend/assets/public/poi-tiles/event-venues/tileset.json'))
  ? viteFsUrl(path.join(runDir, 'frontend/assets/public/poi-tiles/event-venues/tileset.json')) : new URL('/poi-tiles/event-venues/tileset.json', verificationOrigin).href;
...
test('staged successful snapshot mounts every landmark once and opens the shared panel', async ({ page }) => {
  ...
  page.on('response', (response) => {
    if (/\.(?:json|b3dm|glb)(?:\?|$)/i.test(response.url()) && (response.headers()['content-type'] ?? '').includes('text/html')) htmlAssetFallbacks.push(response.url());
  });
  ...
  await expect(page.locator('.landmark-event-pill')).toHaveCount(landmarks.length, { timeout: 45_000 });
  const mountedPillIds = await page.locator('.landmark-event-pill').evaluateAll((nodes) =>
    nodes.map((node) => node.id).toSorted(),
  );
  expect(mountedPillIds).toEqual(landmarks.map((landmark) => `${landmark.id}-event-pill`).toSorted());
  if (landmarks.length) {

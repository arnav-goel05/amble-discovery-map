...
const restaurants = [
  ...
  },
  {
    ...
    osm: {
      type: "node",
      id: "43",
      url: "https://www.openstreetmap.org/node/43",
    },
...
async function mockRestaurantApi(
  page,
  { restaurantGate = null, restaurantPayload = null, dealPayload = null } = {},
) {
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (url.pathname === "/api/restaurant-deals") {
      ...
      const selectedDealPayload =
        typeof dealPayload === "function"
          ? dealPayload(requests.dealStatusIds.length)
          : dealPayload;
      return route.fulfill({
        json: selectedDealPayload || {
          restaurantId: url.searchParams.get("id"),
          status: "complete",
          result: {
            status: "success",
            pagesInspected: [
              { url: "https://example.com/promotions", status: "success" },
            ],
            deals: [
              {
                id: "deal-1",
...
test("button searches the visible map area, clusters locations, lists restaurants, and opens deal details", async ({
  page,
}) => {
  ...
  const requests = await mockRestaurantApi(page, {
    restaurantGate,
    dealPayload: (requestCount) =>
      requestCount <= 3
        ? {
            restaurantId: "osm-node-42",
            status: "pending",
            progress: {
              stage: "searching_website",
              label: "Searching for the official website…",
            },
          }
        : null,
  });
  ...
  await expect(page.locator("#restaurant-search-button")).toHaveClass(
    /is-loading/,
  );
  await expect(page.locator("#restaurant-search-button")).toHaveAttribute(
    "aria-label",
    "Searching for restaurants in this area",
  );
  await expect(page.locator("#restaurant-search-button > i")).toBeHidden();
  await expect(page.locator("#restaurant-results")).toBeHidden();
  await expect(page.locator(".restaurant-results__status")).not.toContainText(
  ...
  await expect(page.locator("#restaurant-search-button")).not.toHaveClass(
    /is-loading/,
  );
  await expect(page.locator(".restaurant-results__count")).toHaveCount(0);
  await expect(page.locator(".restaurant-results__close")).toHaveAttribute(
    "aria-label",
  ...
  await expect(page.locator(".restaurant-results__breadcrumbs")).toContainText(
    "Cafe",
  );
  await expect(cuisineFilter.locator('option[value="british"]')).toHaveText(
    "British · 1",
  );
  ...
  expect(
    await page.evaluate(
      () => window._map.getSource("viewport-restaurants")._data.features.length,
    ),
  ).toBe(1);
  ...
  expect(north - mapStateBeforeSearch.center[1]).toBeCloseTo(
    mapStateBeforeSearch.center[1] - south,
    5,
  );
  expect(east - mapStateBeforeSearch.center[0]).toBeCloseTo(
    mapStateBeforeSearch.center[0] - west,
    5,
  );
  ...
  const mapState = await page.evaluate(() => ({
    ...
    hasClusterCounts: Boolean(
      window._map.getLayer("viewport-restaurant-cluster-count"),
    ),
    hasPoints: Boolean(window._map.getLayer("viewport-restaurant-points")),
    clusterColor: window._map.getPaintProperty(
      "viewport-restaurant-clusters",
      "circle-color",
    ),
    clusterRadius: window._map.getPaintProperty(
      "viewport-restaurant-clusters",
      "circle-radius",
    ),
    pointColor: window._map.getPaintProperty(
      "viewport-restaurant-points",
      "circle-color",
    ),
    pointRadius: window._map.getPaintProperty(
      "viewport-restaurant-points",
      "circle-radius",
    ),
  }));
  ...
  await expect(
    page.locator(".restaurant-detail__deal-status--loading"),
  ).toHaveText("Searching for the official website…");
  await expect(
    page.locator(".restaurant-detail__deal-status--loading"),
  ...
  await expect(page.locator("#restaurant-detail-title")).toHaveText(
    "Example Kitchen",
  );
  await expect(page.locator(".restaurant-detail__kicker")).toHaveCount(0);
  await expect(
    page.locator(".restaurant-detail__actions .restaurant-detail__plan"),
  ).toHaveAttribute("aria-label", "Add restaurant to plan");
  await expect(page.locator(".restaurant-detail__header-link")).toHaveAttribute(
    "href",
    "https://example.com/",
  );
  await expect(page.locator(".restaurant-detail__directions")).toHaveAttribute(
    "aria-label",
    "Get directions to restaurant",
  );
  await expect(page.locator(".restaurant-detail__directions")).toHaveAttribute(
    "href",
    "https://www.google.com/maps/dir/?api=1&destination=1.285%2C103.858",
  );
  await expect(
    page.locator(".restaurant-detail__label", {
      hasText: /^(Website|Map data)$/,
    }),
  ).toHaveCount(0);
  await expect(
    page.locator(
      ".restaurant-detail__field--reference .restaurant-detail__label",
    ),
  ).toHaveText("Reference");
  await expect(
    page.locator(
      ".restaurant-detail__field--reference .restaurant-detail__link",
    ),
  ...
  ).toHaveAttribute("href", "https://www.openstreetmap.org/node/42");
  const actionBoxes = await page
    .locator(".restaurant-detail__actions .restaurant-detail__action")
    .evaluateAll((actions) =>
      actions.map((action) => {
  ...
  expect(
    actionBoxes.every(({ width, height }) => width === 42 && height === 42),
  ).toBe(true);
  const headerLayout = await page
    .locator(".restaurant-detail__header")
    .evaluate((header) => {
      const controlsBottom = Math.max(
        header.querySelector(".restaurant-detail__back").getBoundingClientRect()
          .bottom,
        header
          .querySelector(".restaurant-detail__actions")
          .getBoundingClientRect().bottom,
      );
      return {
        controlsBottom,
        titleTop: header
          .querySelector(".restaurant-detail__title")
  ...
  await expect(detailValues.nth(3)).toHaveText(
    "3 Example Road, Singapore 018900",
  );
  await expect(page.locator(".restaurant-detail__hours-days")).toHaveText(
    "Monday–Sunday",
  );
  await expect(page.locator(".restaurant-detail__hours-time")).toHaveText(
    "11:00–22:00",
  );
  await expect(page.locator(".restaurant-detail__deal-evidence")).toHaveText(
    "Enjoy 20% off dinner from Monday to Thursday.",
  );
  await expect(page.locator(".restaurant-detail__deal-source")).toHaveAttribute(
    "href",
  ...
  await page
    ...
    .evaluate((button) => button.click());
  await expect(page.locator("#plan-builder-button")).toHaveAttribute(
    "aria-label",
    "Plan, 1 stop",
  );
  await expect(
    page.locator(
      ".plan-builder__stop:not(.plan-builder__stop--origin) .plan-builder__stop-title",
    ),
  ).toHaveText("Example Kitchen");
...
test("stale restaurant and deal envelopes are labelled and expired deals stay hidden", async ({
  page,
}) => {
  ...
  await mockRestaurantApi(page, {
    restaurantPayload: {
      ...
      fetchedAt,
      stale: true,
      warning: "live source unavailable",
        ...
        deals: [
          {
            ...
            validUntil: "2026-01-01T23:59:59.999Z",
          },
        ],
      },
    },
  ...
  await expect(page.locator(".restaurant-detail__stale")).toHaveText(
    "Potentially outdated",
  );
  await expect(
    page.locator(".restaurant-detail__deal", { hasText: "Old deal" }),
  ).toHaveCount(0);
  await expect(page.locator(".restaurant-detail__deal-status")).toContainText(
    "No current deals",
  );
});

test("restaurant names are rendered as text and the list remains usable on a narrow viewport", async ({
  page,
}) => {
  ...
  await expect
    ...
    .toBe("mounted");
  await page.locator("#restaurant-search-button").click();
  const unsafe = page.locator(
    '[data-restaurant-id="osm-node-43"] .restaurant-results__name',
  );
  ...
  const backButton = page.getByRole("button", {
    name: "Back to restaurant results",
  });
  ...
  await expect(page.locator("#restaurant-results")).toBeVisible();
  await expect(
    page.locator('[data-restaurant-id="osm-node-43"]'),
  ).toBeFocused();
});

test("restaurant detail gives the name its own row and validates the directions link", async ({
  page,
}) => {
  await page.goto("/test-harness.html");
  const state = await page.evaluate(async () => {
    const { createRestaurantDetail } =
      await import("/activity-scenes/restaurants/restaurant-detail.js");
    const detail = createRestaurantDetail();
    detail.open(
      { name: "Example Kitchen", latitude: 1.285, longitude: 103.858 },
    ...
    const result = {
      ...
      nameHasOwnRow:
        header
          .querySelector(".restaurant-detail__title")
          .getBoundingClientRect().top > controlsBottom,
    };
    detail.open(
      { name: "Missing location" },
      document.getElementById("map-focus"),
    );
    result.directionsHiddenWithoutCoordinates = header.querySelector(
      ".restaurant-detail__directions",
    ).hidden;
    detail.destroy();
    return result;
  });

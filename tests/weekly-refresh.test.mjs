...
} from "../scripts/run-weekly-refresh.mjs";
...
const config = {
  ...
  event: {
    command: ["npm", "run", "event-pipeline", "--", "start"],
    maximumContinuations: 5,
  },
  restaurants: {
    command: ["npm", "run", "restaurant-pipeline", "--", "start"],
    maximumContinuationsPerCoverage: 2,
    coverage: [
      { id: "west", bbox: "1.2,103.7,1.3,103.8" },
      { id: "east", bbox: "1.2,103.8,1.3,103.9" },
    ],
  },
};
const temp = () =>
  fs.mkdtempSync(path.join(os.tmpdir(), "weekly-refresh-test-"));
const response = (payload, status = 0) => ({
  status,
...
test("weekly refresh owns one lock and rejects an overlapping invocation", () => {
  ...
  const result = runWeeklyRefresh({
    root,
    config,
    env: { WEEKLY_REFRESH_OUTPUT_ROOT: output },
  });
...
test("weekly refresh follows the exact event continuation before every configured restaurant coverage", () => {
  ...
  const invoke = (call) => {
        ...
        ? response(
            {
              ...
              complete: false,
              next: {
                command: "npm run event-pipeline -- advance --run event-1",
              },
            },
            3,
          )
    ...
    return response({
      runId: `restaurant-${call.coverageId}`,
      status: "complete",
      complete: true,
    });
  };
  const result = runWeeklyRefresh({
    ...
    config,
    invoke,
    env: { WEEKLY_REFRESH_OUTPUT_ROOT: path.join(root, "runtime") },
  });
  assert.equal(result.complete, true);
  assert.deepEqual(
    calls.map(({ domain, coverageId }) => [domain, coverageId || null]),
    [
      ["events", null],
      ["events", null],
      ["restaurants", "west"],
  ...
  assert.equal(
    JSON.parse(fs.readFileSync(path.join(root, "runtime/latest.json"), "utf8"))
      .status,
    "success",
  );
});
...
test("an event release failure is distinct from a stale preserved snapshot", () => {
  ...
  const result = runWeeklyRefresh({
    ...
    env: { WEEKLY_REFRESH_OUTPUT_ROOT: path.join(root, "runtime") },
    invoke: () =>
      response({ runId: "event-failed", status: "failed", complete: true }, 1),
  });
...
test("restaurant continuation and partial coverage produce a versioned combined terminal report", () => {
  ...
  const result = runWeeklyRefresh({
    ...
    invoke: (call) => {
      ...
      if (call.coverageId === "west" && west++ === 0)
        return response(
          { runId: "west-1", status: "continuation_required", complete: false },
          3,
        );
      if (call.coverageId === "west")
        return response({
          runId: "west-1",
          status: "complete",
          complete: true,
      ...
      return response(
        { runId: "east-1", status: "failed", complete: false },
        1,
      );
    },
  });
  ...
  assert.ok(
    fs.existsSync(path.join(root, "runtime/runs", result.runId, "status.json")),
  );
...
test("checked-in coverage remains bounded and non-overlapping by identity", () => {
  const checkedIn = JSON.parse(
    fs.readFileSync(
      new URL("../data/weekly-refresh-config.json", import.meta.url),
      "utf8",
    ),
  );
  assert.equal(validateWeeklyConfig(checkedIn), checkedIn);
  assert.equal(
    new Set(checkedIn.restaurants.coverage.map(({ id }) => id)).size,
    checkedIn.restaurants.coverage.length,
  );

...
const supportFor = ({ width, height, userAgent, mobile, maxTouchPoints = 0, media = true, socket = true }) => getDeviceSupport({
  screen: { width, height },
  navigator: {
    userAgent,
    maxTouchPoints,
...
test("device support allows laptop, desktop, phone, and tablet browsers", () => {
  ...
  assert.equal(supportFor({ width: 430, height: 932, userAgent: "Mozilla/5.0 iPhone", mobile: true }).supported, true);
  assert.equal(supportFor({ width: 1024, height: 1366, userAgent: "Mozilla/5.0 Macintosh", mobile: false, maxTouchPoints: 5 }).supported, true);
});

test("missing voice capabilities degrades to text and direct controls instead of blocking the app", () => {
  const degraded = supportFor({ width: 430, height: 932, userAgent: "Mozilla/5.0 iPhone", mobile: true, media: false });
  assert.equal(degraded.supported, true);
  assert.equal(degraded.mode, "degraded");
  assert.equal(degraded.voiceSupported, false);
  assert.deepEqual(degraded.missingCapabilities, ["audio-capture", "audio-output"]);
});

test("missing realtime transport preserves direct mode and identifies the missing capability", () => {
  const degraded = supportFor({ width: 900, height: 700, userAgent: "Mozilla/5.0 Linux", mobile: false, socket: false });
  assert.equal(degraded.supported, true);
  assert.equal(degraded.mode, "degraded");

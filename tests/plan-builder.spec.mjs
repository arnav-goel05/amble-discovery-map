...
test("the empty planner explains unavailable actions without duplicate warnings", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("button", { name: "Open route in Google Maps" }),
  ).toBeDisabled();
  await expect(
    page.locator(".plan-builder__telegram, .plan-builder__game-settings"),
  ).toHaveCount(0);
  await expect(page.getByText(/Telegram/i)).toHaveCount(0);
});

test("the MVP planner stays frontend-only and exposes only Google Maps routing", async ({
  page,
}) => {
    ...
    addPlanBuilder({
      geolocation: {
        getCurrentPosition: (resolve) =>
          resolve({ coords: { latitude: 1.283, longitude: 103.851 } }),
      },
    });
  ...
  await expect(page.locator(".plan-builder__preview-summary")).toContainText(
    "1 stop",
  );
...
test("users build, reorder, and route a mixed plan without generating a share link", async ({
  page,
}, testInfo) => {
  let planRequests = 0;
  ...
  await page.evaluate(async () => {
    ...
    window.__planner = addPlanBuilder({
      map: {
        getZoom: () => 14,
        easeTo: (options) => window.__mapMoves.push(options),
      },
      geolocation: {
        getCurrentPosition: (resolve) =>
          resolve({ coords: { latitude: 1.283, longitude: 103.851 } }),
    ...
    const add = (detail) =>
      window.dispatchEvent(
        new CustomEvent("whats-here:add-to-plan", { detail }),
      );
    add({
      id: "event-1",
      type: "event",
  ...
  await expect(
    page.locator(".plan-builder__stop--origin .plan-builder__stop-title"),
  ).toHaveText("My location");
  await expect(
    page.locator(".plan-builder__stop:not(.plan-builder__stop--origin)"),
  ).toHaveCount(2);
  await expect(page.locator(".plan-builder__preview-summary")).toContainText(
    "2 stops",
  );
  ...
  await expect(
    page.locator(".plan-builder__preview-warnings"),
  ).not.toContainText("Accessibility details");
  ...
  await expect(
    page.getByLabel("Remove Example Kitchen").locator(".ph-x"),
  ).toHaveCount(1);
  const dragHandle = page.getByLabel("Drag Example Kitchen to reorder");
  const firstStop = page
    .locator(".plan-builder__stop:not(.plan-builder__stop--origin)")
    .first();
    ...
    await page.evaluate(
      ({ source, target }) => {
        const handle = document.querySelector(
          '[aria-label="Drag Example Kitchen to reorder"]',
        );
        const pointer = {
          bubbles: true,
          cancelable: true,
  ...
  const maps = new URL(
    await page.locator(".plan-builder__maps-link").getAttribute("href"),
  );
  ...
  expect(
    (await page.locator(".plan-builder__maps-link").boundingBox()).height,
  ).toBeLessThanOrEqual(50);
  await expect(
    page.locator(
      ".plan-builder__share-actions, .plan-builder__share--whatsapp",
    ),
  ).toHaveCount(0);
  expect(planRequests).toBe(0);
  await page
    .locator(
      ".plan-builder__stop:not(.plan-builder__stop--origin) .plan-builder__stop-focus",
    )
    .first()
    .click();
  await expect(page.locator("#plan-builder")).toBeVisible();
  expect(await page.evaluate(() => window.__mapMoves)).toEqual([
    { center: [103.858, 1.285], zoom: 17, duration: 700 },
  ]);
});

test("the real plan/game API persists shared plans and authenticates Telegram webhooks", async ({
  request,
}) => {
  ...
  expect(
    (await (await request.get(`/api/plans/${plan.id}`)).json()).title,
  ).toBe("API day out");
  ...
  expect(game.telegramUrl).toBe(
    `https://t.me/WhatsHereTestBot?start=${game.id}`,
  );
  const persistedGame = await (
    await request.get(`/api/games/${game.id}`)
  ).json();
  expect(persistedGame.missions).toHaveLength(1);
  expect(persistedGame.missions[0]).toMatchObject({
    title: "Waterfront show",
    place: "Esplanade",
    latitude: 1.2897,
    longitude: 103.8559,
  ...
  expect(
    (await (await request.get(`/api/games/${game.id}`)).json()).missions[0]
      .title,
  ).toBe("Waterfront show");

  const update = {
    update_id: Date.now(),
    message: { chat: { id: 991122 }, text: `/start ${game.id}` },
  };
  const rejected = await request.post("/api/telegram/webhook", {
    data: update,
    headers: { "X-Telegram-Bot-Api-Secret-Token": "wrong" },
  });
  expect(rejected.status()).toBe(401);
  const accepted = await request.post("/api/telegram/webhook", {
    data: update,
...
test("event details add the selected event and expose its Google Maps destination", async ({
  page,
}) => {
  await page.goto("/test-harness.html");
  await page.evaluate(async () => {
    const [{ addPlanBuilder }, { createLandmarkEventPanel }] =
      await Promise.all([
        import("/activity-scenes/plan-builder.js"),
        import("/activity-scenes/landmark-event-panel.js"),
      ]);
    addPlanBuilder({
      geolocation: {
        getCurrentPosition: (resolve) =>
        ...
        sourceEvents: [
          {
            ...
            dateText: "14 Jul",
            timeText: "8pm",
            eventUrl: "https://example.com/jazz",
          },
        ],
  ...
  expect(await page.locator("#landmark-event-panel").boundingBox()).toEqual(
    planBox,
  );
  ...
  await expect(
    page.locator(
      ".plan-builder__stop:not(.plan-builder__stop--origin) .plan-builder__stop-place",
    ),
  ).toContainText("Esplanade");
  const maps = new URL(
    await page.locator(".plan-builder__maps-link").getAttribute("href"),
  );
  expect(maps.origin + maps.pathname).toBe("https://www.google.com/maps/dir/");
...
test("planner controller publishes stable current plan and game candidates", async ({
  page,
}) => {
  await page.goto("/test-harness.html");
  const snapshots = await page.evaluate(async () => {
    ...
    const revisions = [];
    const unsubscribe = controller.subscribeCandidateState((state) =>
    ...
    controller.setGameCandidates([
      { id: "hunt-2", title: "Night hunt", status: "available" },
    ]);
  ...
  expect(snapshots.afterStop.planStops).toEqual([
  ...
  expect(
    snapshots.afterGames.games.map(({ candidateId }) => candidateId),
  ).toEqual(["game:hunt-2"]);
});
...
test("the route planner remains usable on a phone", async ({ page }) => {
    ...
    for (let index = 0; index < 8; index += 1)
      window.dispatchEvent(
        new CustomEvent("whats-here:add-to-plan", {
          detail: {
            id: `stop-${index}`,
            type: index % 2 ? "restaurant" : "event",
            title: `Stop ${index + 1}`,
            place: `Place ${index + 1}`,
            latitude: 1.28 + index / 1000,
            longitude: 103.85 + index / 1000,
          },
        }),
      );
  });
  await page.evaluate(() => {
    const handles = [
      ...document.querySelectorAll(".plan-builder__drag-handle"),
    ];
    const stops = [

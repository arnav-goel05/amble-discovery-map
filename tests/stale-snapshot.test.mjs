...
function publish(root, snapshotId, overrides = {}) {
  const staged = stageImmutableSnapshot({
    ...
    snapshot: {
      schemaVersion: "1.0",
      snapshotId,
      publishedAt: overrides.publishedAt || "2026-07-14T00:00:00.000Z",
      coveredWindow: {
        start: "2026-07-14",
        end: "2026-07-21",
      ...
      freshness: "fresh",
      staleAfter: overrides.staleAfter || "2026-07-21T00:00:00.000Z",
...
test("approved snapshot API crosses the stale boundary without changing the active pointer", async (context) => {
  ...
  publish(root, "prior", {
    sourceHealth: {
        ...
        status: "failed",
        lastSuccessfulAt: "2026-07-14T00:00:00.000Z",
        error: "SECRET upstream detail",
        internalUrl: "/private/path",
      },
    },
  });
  ...
  assert.deepEqual(payload.data.sourceHealth, {
    catch: { status: "failed", lastSuccessfulAt: "2026-07-14T00:00:00.000Z" },
  });
...
test("verified recovery atomically replaces stale data while no prior snapshot returns unavailable", async (context) => {
  const emptyRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "empty-snapshot-api-"),
  );
  const recoveredRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "recovered-snapshot-api-"),
  );
  context.after(() => {
    fs.rmSync(emptyRoot, { recursive: true, force: true });
    fs.rmSync(recoveredRoot, { recursive: true, force: true });
  });
  const empty = await serverFor(
    emptyRoot,
  ...
  assert.equal(
    (await (await fetch(`${recovered.base}/api/snapshot`)).json()).stale,
    true,
  );
  publish(recoveredRoot, "recovered", {
    publishedAt: "2026-07-22T01:00:00.000Z",
    staleAfter: "2026-07-29T01:00:00.000Z",

...
const authorityRegistry = loadEventAuthorityRegistry(
...
test("approved free and open providers accept only their approved domains", () => {
  assert.equal(
    assertProviderAllowed(policy, "catch-sg", {
      url: "https://www.catch.sg/api/events/SearchListEvent",
    }).costClass,
    "free",
  );
  assert.equal(
    assertProviderAllowed(policy, "openstreetmap-overpass", {
  ...
  assert.throws(
    () =>
      assertProviderAllowed(policy, "catch-sg", {
        url: "https://lookalike.example/events",
      }),
    (error) =>
...
test("unknown, absent-cost, disabled, and paid providers fail closed", () => {
  assert.throws(
    () => assertProviderAllowed(policy, "google-maps"),
    (error) => error.code === "provider_unapproved",
  );
  assert.throws(
    () =>
      assertProviderAllowed(
        {
          schemaVersion: "1.0",
          providers: [{ id: "bad", enabled: true, domains: ["example.test"] }],
        },
        "bad",
      ),
    (error) => error.code === "provider_cost_class_invalid",
  );
  assert.throws(
    () =>
      assertProviderAllowed(
...
test("paid exception assertion rejects every other feature, provider, and domain", () => {
  ...
  assert.throws(
    ...
    (error) =>
      error instanceof ProviderPolicyError &&
      error.code === "provider_unapproved",
  );
});

test("provider provenance is explicit and contains no credentials", () => {
  const provenance = providerProvenance(
    assertProviderAllowed(policy, "sistic"),
    {
      retrievedAt: "2026-07-14T00:00:00.000Z",
      adapterId: "sistic-official-listing-v1",
      adapterVersion: "1.0",
    },
  );
  assert.deepEqual(provenance, {
    providerId: "sistic",
    owner: "SISTIC.com Pte Ltd",
    costClass: "free",
    adapterId: "sistic-official-listing-v1",
    adapterVersion: "1.0",
    retrievedAt: "2026-07-14T00:00:00.000Z",
...
test("event authority registry approves reviewed event paths and reviews unknown domains", () => {
  ...
  assert.throws(
    () =>
      assertAuthorityUrlAllowed(
        authorityRegistry,
        "https://unknown-organizer.example/events/show",
      ),
    (error) => error.code === "authority_domain_review",
  );
  assert.throws(
    () =>
      assertAuthorityUrlAllowed(
        authorityRegistry,
        "https://user:pass@peatix.com/event/show",
      ),
    (error) => error.code === "authority_url_unsafe",
  );

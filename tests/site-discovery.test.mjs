...
test("site identity matches the schema-versioned fixture", () => {
  ...
  assert.equal(SITE_IDENTITY.workerAliasHost, "amble.project-hub-arnav.workers.dev");
});
...
test("canonical redirect matches only configured aliases", () => {
  ...
  assert.equal(
    canonicalRedirect(new URL("https://amble.project-hub-arnav.workers.dev/path?q=1")),
    "https://amblefinds.com/path?q=1",
  );
  assert.equal(

...
test("mixed stops deduplicate by stable type and identity without replacing order", () => {
  ...
  const duplicate = addPlanStop(state, {
    ...event,
  ...
  assert.deepEqual(
    duplicate.state.stops.map(({ id }) => id),
    ["event-1", "food-1"],
  );
...
test("planning candidate state exposes immutable ordered stops and allowlisted games", () => {
  ...
  assert.deepEqual(
    candidates.planStops.map(({ candidateId, position }) => ({
      candidateId,
      position,
    })),
    [
  ...
  assert.deepEqual(
    candidates.games.map(({ candidateId }) => candidateId),
    ["game:hunt-1", "game:hunt-2"],
  );

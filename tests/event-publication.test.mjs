...
test("v2 activity state migrates deterministically into independent v3 dimensions", () => {
  ...
  assert.deepEqual(
    migrated.events.map(
      ({ id, lifecycleState, publicPlacement, mappingStatus, freshness }) => ({
        id,
        lifecycleState,
        publicPlacement,
        mappingStatus,
    ...
    for (const reason of [
      ...
      assert.throws(
        () =>
          stageImmutableSnapshot({
            root: state.root,
            snapshot: approvedSnapshot({ snapshotId: `candidate-${reason}` }),
            artifacts,
            commitEligibility: { eligible: false, reason },
          }),
        new RegExp(reason),
      );
    }
    assert.equal(
      fs.existsSync(path.join(state.root, "data/approved-snapshot.json")),
      false,
    );
  } finally {
    state.cleanup();
  ...
  try {
    ...
    assert.throws(
      () =>
        stageImmutableSnapshot({
          root: state.root,
          snapshot: approvedSnapshot({ snapshotId: "second" }),
          artifacts,
          commitEligibility: { eligible: true },
        }),
      /already exists/i,
...
test("not-mappable accounting may publish when all other gates pass", () => {
  ...
  try {
    const staged = stageImmutableSnapshot({
      root: state.root,
      snapshot: approvedSnapshot({ snapshotId: "safe-not-mappable" }),
      artifacts,
      commitEligibility: { eligible: true, notMappableCount: 1 },
    });
...
test("active snapshots expose stale metadata without changing the approved pointer", () => {
  ...
  try {
    const staged = stageImmutableSnapshot({
      root: state.root,
      snapshot: approvedSnapshot({ snapshotId: "stale" }),
      artifacts,
      commitEligibility: { eligible: true },
    });
    activateStagedSnapshot({ root: state.root, staged });
    const active = loadApprovedSnapshot({
      root: state.root,
...
test("failed activation rolls the pointer back to the previous approved snapshot", () => {
  ...
  try {
    const first = stageImmutableSnapshot({
      root: state.root,
      snapshot: approvedSnapshot({ snapshotId: "rollback-first" }),
      artifacts,
      commitEligibility: { eligible: true },
    });
    activateStagedSnapshot({ root: state.root, staged: first });
    const broken = stageImmutableSnapshot({
      root: state.root,
      snapshot: approvedSnapshot({
        snapshotId: "rollback-broken",
        previousSnapshotId: "rollback-first",
      }),
      artifacts,
      commitEligibility: { eligible: true },
    });
    ...
    assert.throws(
      () => activateStagedSnapshot({ root: state.root, staged: broken }),
...
test("the initial snapshot can be the previous version without changing landmark or POI identities", () => {
  ...
  try {
    ...
    const initial = stageImmutableSnapshot({
      root: state.root,
      snapshot: approvedSnapshot({ snapshotId: "initial" }),
      artifacts: stableArtifacts,
      commitEligibility: { eligible: true },
    });
    activateStagedSnapshot({ root: state.root, staged: initial });
    const next = stageImmutableSnapshot({
      root: state.root,
      snapshot: approvedSnapshot({
        snapshotId: "weekly-next",
        previousSnapshotId: "initial",
      }),
      artifacts: stableArtifacts,
      commitEligibility: { eligible: true },
    });
...
test("commit eligibility and terminal status distinguish finalization from publication", () => {
  ...
  const base = {
    ...
    venues: {
      hall: {
        stages: {
          resolve: { status: "success", resolutionStatus: "approved" },
        },
      },
    },
  };
  assert.deepEqual(evaluateCommitEligibility(base), {
    eligible: true,
    reasons: [],
  });
  assert.equal(deriveTerminalStatus(base), "success");
  const review = structuredClone(base);
  review.venues.hall.stages.resolve = {
    status: "unresolved",
...
test("source reconciliation carries stale contributions per identity, accepts compatible current fields, and archives complete-source removals", () => {
  const previous = [
    {
      id: "mixed",
      identityAnchor: "mixed",
      publishedEventId: "mixed",
      title: "Old title",
      ...
      sourceContributions: [
        {
          sourceRecordId: "Catch:one",
          sourceName: "Catch",
          freshness: "current",
          fields: ["title", "schedule"],
        },
        {
          sourceRecordId: "SISTIC:two",
          sourceName: "SISTIC",
          freshness: "current",
          fields: ["location"],
        },
    ...
    {
      id: "removed",
      identityAnchor: "removed",
      publishedEventId: "removed",
      title: "Removed",
      schedule: { kind: "anytime" },
      sourceContributions: [
        {
          sourceRecordId: "Catch:removed",
          sourceName: "Catch",
          freshness: "current",
          fields: ["title"],
        },
  ...
  const current = [
    {
      ...previous[0],
      title: "Current title",
      sources: [{ source: "Catch", sourceId: "one" }],
      sourceContributions: [previous[0].sourceContributions[0]],
    },
  ];
  const result = reconcileSourceAvailability({
    previousEvents: previous,
    currentEvents: current,
    sourceStatuses: { Catch: "success", SISTIC: "blocked" },
    asOf: "2026-07-18T00:00:00+08:00",
  });
  assert.equal(result.events.length, 1);
  assert.equal(result.events[0].title, "Current title");
  assert.equal(
    result.events[0].sourceContributions.find(
      ({ sourceName }) => sourceName === "SISTIC",
    ).freshness,
    "stale",
  );
...
test("candidate assembly keeps lifecycle, placement, mapping, and freshness orthogonal and rejects unsafe release geometry", () => {
  ...
  assert.throws(
    () =>
      assembleCandidateSnapshot({
        events: [
          { id: "dup", lifecycleState: "held", publicPlacement: "none" },
          { id: "dup", lifecycleState: "held", publicPlacement: "none" },
        ],
      }),
    /duplicated/i,
...
test("accounted source outages and isolated location reviews can publish safe updates while release gates remain atomic", () => {
  ...
  const state = {
    sources: { Catch: { status: "success" }, SISTIC: { status: "blocked" } },
    normalization: {
      status: "success",
      sourceReconciliation: { accounted: true },
    },
    deduplication: { status: "success", isolatedReviewEventIds: ["held"] },
    resolutionPreparation: { status: "success" },
    verification,
    venues: {
      review: {
  ...
  assert.deepEqual(evaluateCommitEligibility(state), {
    eligible: true,
    reasons: [],
  });
  assert.equal(deriveTerminalStatus(state), "success");
  const malformed = structuredClone(state);
  malformed.verification.browser.status = "failed";
  assert.ok(
    evaluateCommitEligibility(malformed).reasons.includes("browser_failed"),
  );
});
...
test("pilot failures are visible but non-blocking while required discovery and dedup gaps block", () => {
  ...
  const dedupBlocked = structuredClone(base);
  dedupBlocked.deduplication = { status: "blocked" };
  assert.ok(
    evaluateCommitEligibility(dedupBlocked).reasons.includes(
      "deduplication_incomplete",
    ),
  );
});

...
} from "../scripts/lib/event-sources/deduplicate.mjs";

const event = (source, id, overrides = {}) => ({
  id: `${source}:${id}`,
  occurrenceId: `${source}:${id}`,
  identityAnchor: `${source}:${id}`,
  title: "Singapore Night Festival 2026",
  startDateTime: "2026-07-17T19:00:00+08:00",
  endDateTime: "2026-07-17T21:00:00+08:00",
  venueId: `${source}-venue`,
  venue: "National Gallery Singapore",
  sources: [{ source, sourceId: id, recordRef: `raw/${source}/${id}.json` }],
  provenanceRefs: [`raw/${source}/${id}.json`],
...
test("nine-source candidate generation preserves edition years and schedule conflicts", () => {
  ...
  assert.deepEqual(
    candidates.map(({ occurrenceIds }) => occurrenceIds),
    [["Catch.sg:one", "SISTIC:two"]],
  );
...
test("post-venue finalization merges only the same approved POI and keeps weak/different locations distinct", () => {
  ...
  const result = finalizeDeduplication({
    ...
    resolutions: {
      ...
      "Fever Singapore-venue": approved("poi-b"),
    },
    sourcePrecedence: { "Catch.sg": 10, SISTIC: 20, "Fever Singapore": 30 },
  });
  assert.equal(result.events.length, 2);
  assert.equal(result.counts.crossSourceDuplicateCollapsed, 1);
  assert.equal(
    result.events.find(
      ({ identityAnchor }) => identityAnchor === "Catch.sg:one",
    ).sources.length,
    2,
  );
  assert.ok(
    result.decisions.some(
      ({ decision }) => decision === "potential_duplicate_kept_distinct",
    ),
  );
});
...
test("sibling performances remain distinct even at one approved venue", () => {
  const events = [
    event("SISTIC", "matinee", {
      startDateTime: "2026-07-17T14:00:00+08:00",
      endDateTime: "2026-07-17T16:00:00+08:00",
    }),
    event("SISTIC", "evening"),
  ];
  assert.equal(generateDedupCandidates(events).length, 0);
  assert.equal(
    finalizeDeduplication({
      events,
      resolutions: { "SISTIC-venue": approved("poi-a") },
    }).events.length,
    2,
  );
});
  ...
  assert.ok(
    result.decisions.every(
      ({ decision, evidence }) =>
        ...
        evidence.reason === "ambiguous_sibling_mapping",
    ),
  );
});
...
test("prior cluster joins block while membership changes preserve the prior anchor", () => {
  ...
  const resolutions = {
    "SISTIC-venue": approved("poi-a"),
    "Fever Singapore-venue": approved("poi-a"),
  };
  const joined = finalizeDeduplication({
    events,
    resolutions,
    priorClusters: [
      { identityAnchor: "old-a", memberIds: ["SISTIC:two"] },
      { identityAnchor: "old-b", memberIds: ["Fever Singapore:three"] },
    ],
  });
  assert.equal(joined.blockingReviews.length, 1);
  assert.equal(joined.events.length, 2);
  const preserved = finalizeDeduplication({
    events,
    resolutions,
    priorClusters: [{ identityAnchor: "old-a", memberIds: ["SISTIC:two"] }],
  });
...
test("same-source category and ticket repeats collapse while preserving child sessions", () => {
  const shared = {
    parentActivityId: "activity:festival",
    parentListingId: "Catch.sg:festival",
    sessions: [{ sessionId: "session:one" }],
    venueOccurrences: [{ venueOccurrenceId: "venue:one" }],
  };
  const events = [
    event("Catch.sg", "category-repeat", shared),
    event("Catch.sg", "ticket-repeat", {
      ...shared,
      title: "Singapore Night Festival — tickets",
      sessions: [{ sessionId: "session:two" }],
    }),
  ];
  const result = finalizeDeduplication({
    events,
    resolutions: { "Catch.sg-venue": approved("poi-a") },
  });
  assert.equal(result.events.length, 1);
  assert.deepEqual(
    result.events[0].sessions.map(({ sessionId }) => sessionId).sort(),
...
test("direct and editorial records merge across all sources while retaining every contribution", () => {
  ...
  assert.deepEqual(
    result.events[0].sourceContributions
      .map(({ sourceRecordId }) => sourceRecordId)
      .sort(),
    ["catch:one", "honey:guide"],
  );
...
test("anytime and off-map matches merge only with the same strong location state", () => {
  ...
  const secret = finalizeDeduplication({
    events: [
      event("Fever Singapore", "secret", anytime),
      event("Time Out Singapore", "secret", anytime),
    ],
  });
  ...
  assert.equal(
    finalizeDeduplication({
      events: [event("Fever Singapore", "secret", anytime), multiple],
    }).events.length,
    2,
  );
});
...
test("generic titles, sibling sessions, distinct editions, and uncertain locations stay distinct", () => {
  assert.equal(
    generateDedupCandidates([
      event("Catch.sg", "generic", { title: "Workshop" }),
      event("SISTIC", "generic", { title: "Workshop" }),
    ]).length,
    0,
  );
  assert.equal(
    generateDedupCandidates([
      event("Catch.sg", "2026"),
      event("SISTIC", "2027", { title: "Singapore Night Festival 2027" }),
    ]).length,
    0,
  );
  const siblings = [
    event("Catch.sg", "matinee", {
      parentActivityId: "activity:show",
      startDateTime: "2026-07-17T14:00:00+08:00",
      endDateTime: "2026-07-17T16:00:00+08:00",
    }),
    event("Catch.sg", "evening", { parentActivityId: "activity:show" }),
  ];
  assert.equal(finalizeDeduplication({ events: siblings }).events.length, 2);
  assert.equal(
    finalizeDeduplication({
      events: [event("Catch.sg", "one"), event("SISTIC", "two")],
      resolutions: {},
    }).events.length,
    2,
  );

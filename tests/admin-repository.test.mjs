...
const { AdminRepository, AdminRepositoryError } = require("../scripts/lib/admin-repository.cjs");
const { AdminAuthService, hashAdminPassword } = require("../scripts/lib/admin-auth-service.cjs");
...
test("admin migrations are restart-safe and sessions expire or revoke server-side", (context) => {
  ...
  assert.deepEqual(repository.db.prepare("SELECT version FROM admin_schema_migrations ORDER BY version").all().map(({ version }) => version), [1]);
  const auth = new AdminAuthService({ repository, passwordHash: hashAdminPassword("correct horse"), clock: state.clock, sessionTtlMs: 1_000 });
  ...
  assert.throws(() => auth.authenticate(first.sessionToken), { code: "admin_session_invalid" });
  const second = auth.login("correct horse", "127.0.0.1");
  state.advance(1_001);
  assert.throws(() => auth.authenticate(second.sessionToken), { code: "admin_session_invalid" });
  repository.close();

  const restarted = new AdminRepository(state);
  assert.deepEqual(restarted.db.prepare("SELECT version FROM admin_schema_migrations ORDER BY version").all().map(({ version }) => version), [1]);
...
test("venue review state is durable, stale-safe, superseding, and idempotent", (context) => {
  ...
  const review = service.createVenueReview({ venueId: "venue-1", evidenceHash, evidenceSnapshot: { venue: "Library", rawNames: ["Library"] }, candidates: [candidate] });
  const duplicate = service.createVenueReview({ venueId: "venue-1", evidenceHash, evidenceSnapshot: { venue: "ignored" }, candidates: [] });
  assert.equal(duplicate.reviewId, review.reviewId);
  assert.equal(repository.listVenueReviews().records.length, 1);
  assert.throws(() => service.decideVenueReview(review.reviewId, { decision: "approve", evidenceHash: "b".repeat(64), candidateGmlId: candidate.gmlId, reason: "Verified", idempotencyKey: "decision-1" }), { code: "venue_review_stale" });
  assert.throws(() => service.decideVenueReview(review.reviewId, { decision: "approve", evidenceHash, candidateGmlId: "unknown", reason: "Verified", idempotencyKey: "decision-2" }), { code: "venue_review_candidate_invalid" });
  const result = service.decideVenueReview(review.reviewId, { decision: "approve", evidenceHash, candidateGmlId: candidate.gmlId, reason: "Official address and local geometry agree", idempotencyKey: "decision-3" });
  assert.equal(result.pipelineReconciliationRequired, true);
  assert.equal(result.review.status, "approved");
  assert.equal(service.decideVenueReview(review.reviewId, { decision: "approve", evidenceHash, candidateGmlId: candidate.gmlId, reason: "Official address and local geometry agree", idempotencyKey: "decision-3" }).idempotent, true);
  assert.throws(() => service.decideVenueReview(review.reviewId, { decision: "approve", evidenceHash, candidateGmlId: candidate.gmlId, reason: "Replay under another key", idempotencyKey: "decision-4" }), { code: "venue_review_already_decided" });
  repository.close();

  const restarted = new AdminRepository(state);
  assert.equal(restarted.getVenueReview(review.reviewId).status, "approved");
  assert.equal(restarted.approvedVenueReviews()[0].decisionCandidateGmlId, candidate.gmlId);
  restarted.close();
});
...
test("new evidence supersedes only unresolved work and validates idempotency operation ownership", (context) => {
  ...
  const repository = new AdminRepository(state);
  const service = new AdminService({ repository });
  const first = service.createVenueReview({ venueId: "venue-2", evidenceHash: "c".repeat(64), evidenceSnapshot: { venue: "A" }, candidates: [candidate] });
  const next = service.createVenueReview({ venueId: "venue-2", evidenceHash: "d".repeat(64), evidenceSnapshot: { venue: "A" }, candidates: [candidate] });
  assert.equal(repository.getVenueReview(first.reviewId).status, "superseded");
  service.decideVenueReview(next.reviewId, { decision: "defer", evidenceHash: next.evidenceHash, reason: "", idempotencyKey: "shared-key" });
  const other = service.createVenueReview({ venueId: "venue-3", evidenceHash: "e".repeat(64), evidenceSnapshot: { venue: "B" }, candidates: [candidate] });

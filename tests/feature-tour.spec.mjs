...
test.beforeEach(async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    document.body.innerHTML = `
      ...
      <div class="landmark-event-search__builder"><input id="landmark-event-search-input"></div>
...
test("walks through every primary feature and remembers completion", async ({
  page,
}) => {
  await page.evaluate(async () => {
    const { createFeatureTour } =
      await import("/activity-scenes/feature-tour.js");
  ...
  await expect(
    page.getByRole("heading", { name: "Events live on the map" }),
  ).toBeVisible();
  await expect(tour).toContainText("1 of 7");

  const next = page.getByRole("button", { name: /Next/ });
  for (const title of [
    "Filter events your way",
...
test("supports back, escape, skip, and a forced replay", async ({ page }) => {
  ...
  expect(
    await page.evaluate(() => window.__featureTour.start({ force: true })),
  ).toBe(true);

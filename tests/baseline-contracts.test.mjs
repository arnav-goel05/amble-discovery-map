...
test("source records require immutable provenance and an inclusive window", () => {
  assert.equal(
    validateSourceRecord(sourceRecord()).listingIdentity,
    "fixture-listing-1",
  );
  rejectsWith(
    () =>
      validateSourceRecord(
        sourceRecord({ recordPointer: "https://example.test/raw.json" }),
      ),
...
test("canonical occurrences keep occurrence, parent, and merged identities separate", () => {
  const occurrence = {
    ...
    parentListingId: "fixture:list-1",
    mergedEventId: "merged:fixture",
    ...
    description: null,
    category: null,
    officialUrl: "https://example.test/events/1",
    contentHash: hash("d"),
    provenanceRefs: ["raw/fixture/listing.json#/records/0"],
    reviewStatus: "eligible",
  };
  assert.equal(
    validateEventOccurrence(occurrence).occurrenceId,
  ...
  rejectsWith(
    () =>
      validateEventOccurrence({
        ...occurrence,
        occurrenceId: occurrence.parentListingId,
      }),
...
test("venue evidence and review contracts enforce partition and evidence-bound decisions", () => {
  assert.equal(
    validateVenueEvidence(venueEvidence()).resolutionStatus,
    "pending",
  );
  rejectsWith(
    () =>
      validateVenueEvidence(
        venueEvidence({ eventIds: ["event-1", "event-1"] }),
      ),
    "venue_event_partition_invalid",
  );
  ...
  const review = {
    ...
    evidenceHash: hash("b"),
    evidenceSnapshot: {},
    candidates: [{ gmlId: "SLA_BLDG2_1" }],
    status: "approved",
    decisionCandidateGmlId: "SLA_BLDG2_1",
    decisionReason: "Unique official-address match",
  ...
  rejectsWith(
    () =>
      validateVenueReview({
        ...review,
        decisionCandidateGmlId: "SLA_BLDG2_UNKNOWN",
      }),
...
test("snapshot and result envelopes reject incomplete or misleading states", () => {
  ...
  const result = {
    ...
    stale: false,
    warning: null,
    source: { id: "fixture", costClass: "free" },
  };
  ...
  rejectsWith(
    () => validateResultEnvelope({ ...result, stale: true, warning: null }),

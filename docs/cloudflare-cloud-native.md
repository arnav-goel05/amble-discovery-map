...
| --------------------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------- |
| Ordinary CI           | Push to `develop` or an explicitly created non-main branch   | 592-byte checked-in geometry contract; zero production hydration, R2 requests, provider calls, or deployment | Lint, changed-file formatting, every Node test, event/voice contracts, local Cloudflare worker contracts, and the production-equivalent frontend build | None                                                                   |
| Release verification  | Explicit dispatch with the full current `origin/develop` SHA | One approved-geometry hydration, bounded R2 control-plane/inventory checks, no public object-head loop       | Ordinary gates plus production geometry/separation, production build, and staged Chromium event-pipeline integration                                   | Fast-forward `main` to the exact tested SHA after refs are revalidated |

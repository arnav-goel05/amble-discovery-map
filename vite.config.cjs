  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use(async (request, response, next) => {
        ...
        if (!origin || !["GET", "HEAD"].includes(request.method) || !TILE_PATH.test(url.pathname)) return next();

        const localPath = path.resolve(process.cwd(), "public", `.${url.pathname}`);
        const publicRoot = path.resolve(process.cwd(), "public");
        if (localPath.startsWith(`${publicRoot}${path.sep}`) && fs.existsSync(localPath)) return next();
        ...
        try {
          ...
          server.config.logger.warn(`Tile fallback failed for ${url.pathname}: ${error.message}`);
          next();
        }
      });
    },

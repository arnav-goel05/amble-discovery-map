...
function json(
  payload,
  { status = 200, cacheControl = "no-store", headers = {} } = {},
) {
...
function successEnvelope(
  data,
  {
    ...
    stale = false,
    warning = null,
    source = { id: "amble", costClass: "free" },
  } = {},
) {
...
function snapshotMetadata(now = new Date()) {
  ...
  const stale =
    Number.isFinite(Date.parse(manifest.staleAfter)) &&
    now.getTime() > Date.parse(manifest.staleAfter);
  const sourceHealth = Object.fromEntries(
    Object.entries(manifest.sourceHealth || {}).map(([id, health]) => [
      id,
      {
        status: [
          "success",
        ...
        !Number.isNaN(Date.parse(health.lastSuccessfulAt))
...
function snapshotResponse(request, url) {
  ...
  if (!["GET", "HEAD"].includes(request.method))
    return json(
      errorEnvelope("method_not_allowed", "Only GET and HEAD are supported."),
  ...
  if (url.pathname === "/api/snapshot") {
    const body = successEnvelope(metadata.data, {
      ...
      warning: metadata.stale
        ? "The latest approved event snapshot may be out of date."
        : null,
      source: { id: "approved-snapshot", costClass: "free" },
    });
    return request.method === "HEAD"
      ? withSecurityHeaders(
          new Response(null, { headers: { "cache-control": "no-cache" } }),
        )
      : json(body, { cacheControl: "no-cache" });
  }
  ...
  if (snapshotId !== manifest.snapshotId || !(reference in assets))
    return json(
      errorEnvelope(
        "snapshot_asset_not_active",
  ...
  if (reference === manifest.tilesetRef) {
    const response = json(publicTileset(assets[reference]), {
      cacheControl: immutable,
    });
    return request.method === "HEAD"
      ? withSecurityHeaders(new Response(null, { headers: response.headers }))
      : response;
  }
  const response = json(
    successEnvelope(assets[reference], {
      fetchedAt: manifest.publishedAt,
      stale: metadata.stale,
      warning: metadata.stale
        ? "The latest approved event snapshot may be out of date."
...
function tag(tags, ...keys) {
  for (const key of keys)
    if (normalizeSpace(tags?.[key])) return normalizeSpace(tags[key]);
  return null;
}
...
function normalizeRestaurant(element) {
  ...
  return {
    ...
    website: normalizeWebsite(
      tag(tags, "website", "contact:website", "brand:website"),
    ),
    ...
    dietary: Object.entries(tags)
      .filter(
        ([key, value]) =>
          key.startsWith("diet:") && ["yes", "only"].includes(value),
      )
...
export function parseBbox(
  input,
  { maxLatSpan = 0.12, maxLngSpan = 0.12 } = {},
) {
  ...
  if (north - south > maxLatSpan || east - west > maxLngSpan)
    throw new Error(
      "viewport is too large; zoom in before searching restaurants",
    );
  return {
    south,
    west,
    north,
    east,
    key: values.map((value) => value.toFixed(5)).join(","),
  };
}
...
function restaurantEnvelope(
  result,
  { cache, stale = false, warning = null } = {},
) {
  return {
    schemaVersion: "1.0",
    status: "success",
...
async function savedRestaurants(bbox, env) {
  const query = await env.RUNTIME_DB.prepare(
    `SELECT payload FROM restaurants
    WHERE latitude BETWEEN ? AND ? AND longitude BETWEEN ? AND ?
    ORDER BY id LIMIT 250`,
  )
    .bind(bbox.south, bbox.north, bbox.west, bbox.east)
    .all();
  const restaurants = (query.results || [])
...
async function restaurantResponse(request, url, env, context) {
  if (request.method !== "GET")
    return json(errorEnvelope("method_not_allowed", "Only GET is supported."), {
      status: 405,
      headers: { allow: "GET" },
    });
  let bbox;
  ...
  const row = await env.RUNTIME_DB.prepare(
    "SELECT payload,fetched_at,expires_at FROM restaurant_viewports WHERE cache_key=?",
  )
    .bind(bbox.key)
    .first();
  if (row && Date.parse(row.expires_at) > now.getTime()) {
    return json(restaurantEnvelope(JSON.parse(row.payload), { cache: "hit" }), {
      cacheControl: "public, max-age=300, stale-while-revalidate=3600",
    });
  }
  const saved = await savedRestaurants(bbox, env);
  if (saved) {
    return json(restaurantEnvelope(saved, { cache: "database" }), {
  ...
  try {
    ...
    await env.RUNTIME_DB.prepare(
      `INSERT INTO restaurant_viewports(cache_key,south,west,north,east,payload,fetched_at,expires_at)
    ...
    context.waitUntil(
      env.RUNTIME_DB.prepare(
        "DELETE FROM restaurant_viewports WHERE expires_at<?",
      )
        .bind(new Date(now.getTime() - 7 * RESTAURANT_TTL_MS).toISOString())
        .run(),
    );
    return json(restaurantEnvelope(result, { cache: "miss" }), {
      cacheControl: "public, max-age=300, stale-while-revalidate=3600",
    });
  } catch {
    if (row)
      return json(
        restaurantEnvelope(JSON.parse(row.payload), {
          cache: "stale",
          stale: true,
          warning:
            "Live restaurant refresh failed; showing the last saved result.",
        }),
...
export default {
  async fetch(request, env, context) {
    ...
    if (discovery) return withSecurityHeaders(discovery);
    ...
    if (tileObjectKey(request)) {
      try {
        ...
        return withSecurityHeaders(
          new Response("Tile service unavailable", { status: 503 }),
        );
      }
    }
    if (
      url.pathname === "/api/snapshot" ||
      url.pathname.startsWith("/api/snapshot/assets/")
    )
    ...
    if (url.pathname === "/api/game-readiness")
      return json({
        enabled: false,
        botConfigured: false,
        deliveryConfigured: false,
        adaptiveLocationVerification: false,
        photoVerification: false,
        storage: "d1",
      });
    if (url.pathname.startsWith("/api/"))
      return json(
        errorEnvelope("route_not_found", "API route was not found."),
        { status: 404 },
      );
